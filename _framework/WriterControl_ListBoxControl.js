...
export let WriterControl_ListBoxControl = {
    ...
    CreateMultiSelectControl: function (listItems, rootElement, divContainer, oldText, oldValue, currentInputProps, args) {
        ...
        if (listItems != null && Array.isArray(listItems) && listItems.length > 0) {
            ...
            if (Array.isArray(listItems) && listItems.length > 0) {
                // 解析oldText和oldValue
                // 给默认值
                // 修改多选项选中逻辑为同时判断Value属性和Text属性【DUWRITER5_0-4384】
            ...
            var MultiSelectFooterVisible = true;
            // 添加编辑器属性DropdownListMultiSelectFooterVisible判断多选下拉底部工具条是否可见【DUWRITER5_0-4628】
            var _MultiSelectFooterVisible = rootElement.getAttribute("DropdownListMultiSelectFooterVisible");
    ...
    /**
     * 执行多选取消操作时的回调函数
     * 该函数的主要目的是在取消多选操作时，关闭相关的下拉列表
     * 
     * @param {HTMLElement} divContainer - 页面上的下拉列表元素
     */
    MultiSelectCancelCallBackFunc: function (divContainer) {
        if (divContainer && typeof divContainer.CloseDropdown === "function") {
     ...
     * 多选下拉框切换全选状态的回调函数
     * @param {Event} e - 事件对象
     * @param {HTMLElement} meunDiv - 多选下拉框的DOM元素
     * @param {Object} currentInputProps - 当前输入框的属性
     * @param {HTMLElement} rootElement - 根元素
     */
    ...
    CheckMultiSelect: function (e, meunDiv, separatorChar, currentInputProps, rootElement) {
        ...
        if (_target != null && _target != meunDiv) {
            ...
            if (_target != null && _target.getAttribute && _target.getAttribute('class') == 'MultiSelect-item') {
                ...
                /** 下拉元素 */
                var divContainer = rootElement.querySelector("#divDropdownContainer20230111");
                if (divContainer) {
                    /**多选下拉的页脚元素*/
                    var MultiSelectFooterNode = divContainer.querySelector("#DCMultiSelectFooter");
    ...
    CreateDropdownCodeArea: function (listBox, rootElement, divContainer, currentInputProps, args) {
        ...
        var EventShowDropdownOnce = function (rootElement, divContainer) {
            if (!divContainer) {
                return;
            }
            // 需要特殊样式
            ...
            if (!DropDownListMaxHeight && listItems.length > 6) {
                ...
                //判断是否需要增加"使用此文本"的高度
                var searchInput = divContainer.querySelector('.fullButtonDiv');
                if (fillButton) {
            ...
            var targetLiNode = listBox.querySelector('[targetLi]');
            if (targetLiNode) {
                // 获取可以滚动的元素
                var scrollElement = targetLiNode.parentNode;
        ...
        if (hasMinCount && hasMinCount <= listItems.length) {
            ...
            var eventObj = {
                ...
                //[DUWRITER5_0-4191] 20250225 lxy 下拉搜索事件中增加AddResultItemByTextValueTextInList方法
                AddResultItemByTextValueTextInList: function (strText, strValue, strTextInList) {
                    eventObj.NewListItems.push({ Text: strText, Value: strValue, TextInList: strTextInList });
                },
                //文本改变后重新显示的数据
    ...
    ChangeListBoxCheck: function (rootElement, e, currentInputProps) {
        ...
        if (divDropdownContainer.style.display != "none") {
            ...
            //}
            // 判断是否为下拉输入域
            var hasListBox = divDropdownContainer.querySelector('.dcListBox,#MultiSelectControl');
            if (!hasListBox) {
                // 不是下拉输入域
                return;
            }
            ...
            if (NowMoveLi) {
                ...
                // 获取可以滚动的元素
                var scrollElement = NowMoveLi.parentNode;
            ...
            if (e.keyCode == 13) {
                ...
                if (isMultiSelect == false) {
                    ...
                    if (rootElement.EventAfterEnter != null && typeof (rootElement.EventAfterEnter) == "function") {
                        e.preventDefault();
                        return rootElement.EventAfterEnter(NowMoveLi, e);
                    }
                }

...
export let WriterControl_EF = {
        ...
        try {
            ...
            switch (strFunctionName.toUpperCase()) {
                ...
                case "SUMINNERVALUE":
                    return this.SUMINNERVALUE(...args);
                    break;
                case "COUNTINNERVALUE":
                    return this.COUNTINNERVALUE(...args);
                    break;
                // default:
    ...
    DAYDIFF2: function () {
        ...
        if (dateStr != null) {
            ...
            dateone = DCTools20221228.strToDate(strs[0]); //分隔字符串，转换为时间
        }
        ...
        if (dateStr != null) {
            ...
            dateStr = dateStr.toString().replace(/-/g, "/");//传入的时间字符串为yyyy-MM-dd hh:mm:ss，全部转换为/
            var strs = dateStr.split(' ');
            datetwo = DCTools20221228.strToDate(strs[0]); //分隔字符串，转换为时间
        }
    ...
    LOOKUP: function () {
        ...
        if (arguments[0] === null || arguments[0] === "" || arguments[0] === "\n") {
            return null;
        }
    ...
    AVERAGE: function () {
        ...
        for (iCount = 0; iCount < arguments.length; iCount++) {
            if (isNaN(arguments[iCount]) == false) {
                var val = parseFloat(arguments[iCount]);
                total = total + val;
                count = count + 1;
            }
        }
    ...
    /**
     * 根据输入域innervalue判断下拉项勾选数量
     */
    COUNTINNERVALUE: function (text) {
        if (typeof (text) === "string") {
            var val = text.split(',').length;
            return val;
        }
        return null;
    },
    ...
    MAX: function () {
        ...
        var iCount = 0;
        for (iCount = 0; iCount < arguments.length; iCount++) {
            //wyc20250217:修复DUWRITER5_0-4170
            var num = Number.NaN;
            if (typeof (arguments[iCount]) === "number") {
                num = arguments[iCount];
            } else {
                num = parseFloat(arguments[iCount]);
            }
            ///////////////////////////////////
            if (isNaN(result)) {
                result = num;

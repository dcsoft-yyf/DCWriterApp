...
export let WriterContorl_FlowChart = {
  ...
  ChangeData: function (rootElement, data) {
    ...
    //计算备注区域高度
    if (Config.Notes && Config.Notes.Visible && Config.Notes.Visible === true) {
      CalculationData['NotesHeight'] = Config.Notes.SpecifyHeight || 160;
    }
  },
  //整理value数据
  ...
  DrawYAxisValueChart: function (rootElement) {
    ...
    var StartingTime = rootElement.DocumentOptions.DefaultData.Config.StartingTime; //开始时间(规律宫缩开始时间)
        ...
        for (var key in Values) {
          ...
          if (thisValue && thisValue.length > 0) {
            ...
            thisValue.forEach((point, pointIndex) => {
              var startTimeStamp = new Date(StartingTime).getTime();//开始时间戳
              var currentTimeStamp = new Date(point.Time).getTime();//当前时间戳
              var timeDiff = currentTimeStamp - startTimeStamp;//时间差
              var colIndex = Math.ceil(timeDiff / 1000 / 60 / 60);//计算第几个小时

              //绘制点信息必须在24小时内
              if (timeDiff >= 0 && colIndex >= 0 && colIndex <= 24) {
                //图例
                var x = point.X;
                var y = point.Y;
                ...
                return console.log("绘点数据超出了24小时：", timeDiff, point);
              }
            });
            // 线段样式
        ...
        if (YAxisValueArray) { // 查找第一个大于等于 TargetYAxisValue 的元素
          for (let item of YAxisValueArray) {
            if (item.Value >= TargetYAxisValue) {
              //并且判断是否在24小时内
              var startTimeStamp = new Date(StartingTime).getTime();
              var currentTimeStamp = new Date(item.Time).getTime();
              var timeDiff = currentTimeStamp - startTimeStamp;
              var colIndex = Math.ceil(timeDiff / 1000 / 60 / 60);
            ...
            if (k !== 'Time' && k !== 'colIndex') {
              ...
              if (targetRect) {
                ...
                if (IsCellSegment && IsCellSegment === "true") {
                  ...
                  //字体可能会被缩小
                  var scaleFontSize = (TextObject && TextObject.fontSize) || FontSize || 12;
  ...
  DrawSvg: function (rootElement) {
    ...
    for (var i = 0; i < CalculationData.Config.TotalPageNumber; i++) {
      ...
      that.DrawFooterLabels(rootElement, svg, svgEle, i);
      that.DrawAttachedTable(rootElement, svg, svgEle, i);
      that.DrawNotes(rootElement, svg, svgEle, i);
  ...
  //绘制备注
  DrawNotes: function (rootElement, svg, svgEle, i) {
    var { CalculationData } = rootElement.DocumentOptions;
    var { Config } = CalculationData;
    if (Config.Notes && Config.Notes.Visible && Config.Notes.Visible === true) {
      var { PageSettings, Notes } = Config;
      //整理备注区域的绘制数据
      var x = PageSettings.LeftMargin + CalculationData.dcLeftYAxisTotalWidth;
      var y = CalculationData.dcAttachedTablYStartPositon + CalculationData.dcAttachedTableTotalHeight;
      CalculationData['NotesStartYStartPositon'] = y;//保留一遍Y轴起始位置
      var width = CalculationData.dcAttachedTableTotalWidth;
      var height = CalculationData.NotesHeight;
      //默认使用表格处的颜色
      var borderColor = (Config.AttachedTable && Config.AttachedTable.TableCellBorderCorlorValue) || '#606266';
      //创建备注区域的G元素
      var NotesG = svg.append("g")
        .attr("dctype", "NotesG")
      ...
      //绘制备注文本
      var notesText = Notes.Title || "";
      var NotesTextG = NotesG.append("g")
        .attr("dctype", "NotesText")
        .attr("transform", `translate(10,0)`)
        .append("text")
        .attr("style", `font-family:${Notes.TitleFontName || Notes.FontName || Config.FontName || '宋体'}; 
                      font-size:${Notes.TitleFontSize || Notes.FontSize || 14}px; 
                      font-weight:${Notes.TitleFontBold || Notes.FontBold ? "bold" : ""}; 
                    `)
        .text(notesText);
      //修正位置，使其居中
      var textHeight = NotesTextG.node().getBBox().height;
      var NodeTextWidth = NotesTextG.node().getBBox().width;
      NotesTextG.attr("transform", `translate(10,${(height + textHeight) / 2})`);

      //备注内容
      var notesValue = Notes.Value || "";
      if (notesValue != '') {
        var NotesValueG = NotesG.append("g")
          .attr("dctype", "NotesValue")
          .attr("transform", `translate(${NodeTextWidth + 31},1)`)
          .attr("width", width - NodeTextWidth - 30)
          .attr("height", height);
        var NotesValueRect = NotesValueG.append("rect")
          .attr("x", 0)
          .attr("y", 0)
          .attr("width", width - NodeTextWidth - 30 - 2)
          .attr("height", height - 2)
          .attr("fill", "#fff");
        var NotesValueRectWidth = NotesValueRect.node().getBBox().width;
        var NotesValueRectHeight = NotesValueRect.node().getBBox().height;

        //写入内容
        var NotesValueGText = NotesValueG.append("text")
          .attr("style", `font-family:${Notes.FontName || Config.FontName || '宋体'}; 
                      font-size:${Notes.FontSize || 14}px; 
                      font-weight:${Notes.FontBold ? "bold" : ""}; 
                    `)
          .text(notesValue);
        var NotesValueGTextWidth = NotesValueGText.node().getBBox().width;
        var NotesValueGTextHeight = NotesValueGText.node().getBBox().height;
        NotesValueGText.attr("x", (NotesValueRectWidth - NotesValueGTextWidth) / 2)
          .attr("y", (NotesValueRectHeight + NotesValueGTextHeight) / 2);


        //超出就换行展示
        if (NotesValueGTextWidth > NotesValueRectWidth) {
          //需要的行数
          var lines = Math.ceil(NotesValueGTextWidth / NotesValueRectWidth);
          //每行的字符数
          var stepsText = notesValue.length / lines;
          //每行的字符数组
          var valueLineArray = [];
          for (var k = 0; k < lines; k++) {
            valueLineArray.push(notesValue.substring(k * stepsText, (k + 1) * stepsText));
          }
          //清除原有的溢出文本
          NotesValueGText.remove();

          //创建一个新的G元素，用于存放每行的文本
          var NotesValueGroupG = NotesValueG.append("g");
          for (var k = 0; k < valueLineArray.length; k++) {
            NotesValueGroupG.append("text")
              .attr("style", `font-family:${Notes.FontName || Config.FontName || '宋体'}; 
                      font-size:${Notes.FontSize || 14}px; 
                      font-weight:${Notes.FontBold ? "bold" : ""}; 
                    `)
              .attr("x", 0)
              .attr("y", (k + 1) * NotesValueGTextHeight)
              .text(valueLineArray[k]);
          }
          var NotesValueGroupGWidth = NotesValueGroupG.node().getBBox().width;
          var NotesValueGroupGHeight = NotesValueGroupG.node().getBBox().height;
          var NotesValueGroupGWidthX = (NotesValueRectWidth - NotesValueGroupGWidth) / 2;
          var NotesValueGroupGWidthY = (NotesValueRectHeight - NotesValueGroupGHeight) / 2;
          //修正位置
          NotesValueGroupG.attr("transform", `translate(${NotesValueGroupGWidthX},${NotesValueGroupGWidthY})`);
        }
      }
      svgEle[i].dcNotesG = NotesG;

      //判断是否需要调整底部标签的位置
      if (Config.FooterLabels && Config.FooterLabels.length > 0) {
        var FooterLabelsY = CalculationData.dcAttachedTablYStartPositon + CalculationData.dcAttachedTableTotalHeight + height;//结束位置
        svgEle[i].dcFooterLabelsG.attr('transform', 'translate(' + CalculationData.dcAttachedTableXStartPositon + ',' + FooterLabelsY + ')');
      }
  ...
  //绘制页脚标题
  DrawFooterLabels: function (rootElement, svg, svgEle, i) {
    var { CalculationData } = rootElement.DocumentOptions;
    var { Config } = CalculationData;

    if (Config.FooterLabels && Config.FooterLabels.length > 0) {
      var { PageSettings, FooterLabels } = Config;

      //先在底部绘制出来后，再修改y的位置
      var y = PageSettings.PaperHeight - (PageSettings.BottomMargin || 0) - (CalculationData.PageNumberHeight || 0) - (CalculationData.FooterDescriptionHeight || 0);
      var x = PageSettings.LeftMargin + CalculationData.dcLeftYAxisTotalWidth;
      var width = CalculationData.dcAttachedTableTotalWidth;
      var FooterLabelsG = svg.append("g")
        .attr("dctype", "FooterLabelsG")
        .attr("transform", `translate(${x},${y})`)
        .attr("width", width);
      //绘制页脚标题
      this.DrawHeaderLabelsTool(CalculationData, FooterLabelsG, FooterLabels, width);
      var FooterLabelsGHeight = FooterLabelsG.node().getBBox().height;
      y -= (FooterLabelsGHeight * 2);
      FooterLabelsG.attr("transform", `translate(${x},${y + 5})`);
      //记录高度
      CalculationData['dcFooterLabelsGHeight'] = FooterLabelsGHeight + 5;
      svgEle[i].dcFooterLabelsG = FooterLabelsG;
    }
  },
  //绘制标题
  ...
  DrawAttachedTable: function (rootElement, svg, svgEle, i) {
    ...
    var dcAttachedTablYStartPositon =
      ...
      (CalculationData.NotesHeight || 0) -
      (CalculationData.dcFooterLabelsGHeight || 0) -
    ...
    //判断是否需要调整底部标签的位置
    if (Config.FooterLabels && Config.FooterLabels.length > 0) {
      var FooterLabelsY = dcAttachedTablYStartPositon + CalculationData.dcAttachedTableTotalHeight;//表格结束位置
      svgEle[i].dcFooterLabelsG.attr('transform', 'translate(' + CalculationData.dcAttachedTableXStartPositon + ',' + FooterLabelsY + ')');
    }
  ...
  DrawYAxisInfos: function (rootElement, svg, svgEle, i) {
    ...
    //减去底部小标题的高度
    if (CalculationData.dcFooterLabelsGHeight) {
      dcYAxisTotalHeight -= CalculationData.dcFooterLabelsGHeight;
    }
          ...
          if (allText && allText.length > 0) {
            for (var k = 0; k < allText.length; k++) {
              ...
              if (k === allText.length - 1) {
                text.setAttribute("x", ((textX + (x * k)) - 2) || 2);
              } else if (k === 0) {
    ...
    function drawYAxisTicks(DrawLeftYAxisG, YAxisData, startXPosition, currentTarget, CalculationData, Config, DrawYAxisG) {
      ...
      for (var j = 0; j < YAxisData.length; j++) {
        ...
        for (var k = 0; k < thisLeftYAxis.YSplitNum; k++) {
          ...
          //整数前面加符号
          if (thisLeftYAxis.SymbolsBeforeInteger) {
            thisLeftYAxisValue = thisLeftYAxisValue >= 1 ? thisLeftYAxis.SymbolsBeforeInteger + thisLeftYAxisValue : thisLeftYAxisValue;
          }
          // 画刻度值
          DrawLeftYAxisG.append("text")
  ...
  DrawYAxisBottomSplitXAxisNumberArray: function (rootElement, svg, svgEle, i, DrawYAxisG) {
    ...
    var DrawYAxisBottomSplitXAxisNumberArrayG = DrawYAxisG.append("g")
      ...
      .attr("transform", `translate(${0},${CalculationData.dcYAxisTotalHeight + 4})`);
  ...
  DrawAttachedTableHeaderLeftYAxis: function (rootElement, svg, svgEle, i, dcAttachedTableG) {
    ...
    if (
      ...
      for (var j = 0; j < TableRowData.length; j++) {
        ...
        if (thisRowData.ShowBorder !== false) {
          //先在顶部画线

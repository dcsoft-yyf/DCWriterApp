...
export let WriterControl_Paint = {
    ...
    */
     ...
     */
    SetPageElementSize: function (rootElement, element, changezoom, length, baseZoomRate, zoomRate) {
        if (rootElement.__DCDisposed == true) {
            return;
        }
        if (typeof (baseZoomRate) != "number") {
            baseZoomRate = rootElement.__DCWriterReference.invokeMethod("get_WASMBaseZoomRate");
            zoomRate = rootElement.__DCWriterReference.invokeMethod("get_ZoomRate");
        }
        var nativeZoomRate = zoomRate;
        //判断标尺是否存在
        ...
        if (typeof hasAutoZoom == "string"
            && hasAutoZoom.toLowerCase().trim() == "true"
            && changezoom
            && hasCanvas.length == length) {
            if (hasCanvas.length > 0) {
                ...
                if (zoomRate <= 0) {
                    zoomRate = nativeZoomRate;// rootElement.__DCWriterReference.invokeMethod("get_ZoomRate");
                }
                        ...
                        for (var pageIndex = 0; pageIndex < es2.length; pageIndex++) {
                            if (es2[pageIndex] == this) {
                                ...
                                if (rootElement.__DCWriterReference.invokeMethod("Is_WaittingForDragStart") == false) {
                                    if (!(rootElement.ReadViewMode || rootElement.Readonly) || e.type != "mousedown") {
                                        e.stopPropagation();
                                        e.preventDefault();
                                        e.returnValue = false;
                                    }

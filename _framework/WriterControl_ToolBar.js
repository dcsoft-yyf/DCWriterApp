...
export let WriterControl_ToolBar = {
    ...
    `;
        ...
        theTool.buildStage = function () {
            ...
            //self.dom.pannel.insertAdjacentElement('afterend', self.dom.maskBg);
        };
        ...
        theTool.buildPanel = function () {
            ...
            for (let i = 0; i < this.MenuBars.length; i++) {
                ...
                menuBtnDOM.onmouseenter = (e) => {
                    ...
                    if (el.name) {
                        e.stopPropagation();
                        let rootElement = DCTools20221228.GetOwnerWriterControl(el);
        ...
        rootElement.EventUpdateToolarState = changeCurrentProp
        theTool.init();
    },
    //隐藏工具条
    ...
    ChangeFontFamilys: function (containerID, fontFamilysList) {
        var rootElement = DCTools20221228.GetOwnerWriterControl(containerID);
        let toolPanel = rootElement.querySelector(".DC-toolBar-panel");
        if (fontFamilysList && fontFamilysList.length < 1) {
            return false
        }
        if (toolPanel) {
            let toolDOM = rootElement.querySelector('div[commendname=fontname]')
            if (toolDOM) {
                let selectDOM = toolDOM.querySelector('select')
                let optionStr = "";
                for (let k = 0; k < fontFamilysList.length; k++) {
                    optionStr += `<option value="${fontFamilysList[k]}">${fontFamilysList[k]}</option> `;
                }
                selectDOM.innerHTML = optionStr;
                return true
            } else {
...
let showMaskBg = function (rootElement) {
    ...
    if (oldMaskBgDOM) {
        ...
        /*//rootElement.ownerDocument.body.appendChild(newMaskBgDOM);*/
        // pageContainer.insertAdjacentElement('beforeend', newMaskBgDOM);
    }
    
    // 显示全局遮罩层
    showGlobalMask(rootElement);
};
 ...
 * 显示全局遮罩层，用于处理Menu关闭
 */
let showGlobalMask = function (rootElement) {
    // 检查是否已存在全局遮罩层
    let globalMask = rootElement.ownerDocument.querySelector("#dc-global-menu-mask");
    if (!globalMask) {
        globalMask = document.createElement("div");
        globalMask.id = "dc-global-menu-mask";
        globalMask.dcWriterID = rootElement.id;
        globalMask.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
            ...
            pointer-events: auto;
        `;
        rootElement.ownerDocument.body.appendChild(globalMask);

        // 添加点击事件，关闭所有Menu
        globalMask.addEventListener("click", function (e) {
            let ctl = DCTools20221228.GetOwnerWriterControl(e.target.dcWriterID) ?? rootElement
            hiddenMenu(ctl);
        });
    } else {
        globalMask.dcWriterID = rootElement.id
 ...
 * 隐藏全局遮罩层
 */
let hideGlobalMask = function (rootElement) {
    let globalMask = rootElement.ownerDocument.querySelector("#dc-global-menu-mask");
    if (globalMask) {
...
/**
 * 隐藏所有WriterControl实例的Menu
 */
let hiddenAllMenus = function () {
    console.log('hiddenAllMenus');
    
    // 获取所有WriterControl实例
    WriterControl_UI.GetCurrentWriterControl(function (allWriterControls) {
        for (let i = 0; i < allWriterControls.length; i++) {
            const writerControl = allWriterControls[i];
            hiddenMenu(writerControl);
        }
    });
};
let build_ToolBar = function (rootElement) {
    let toolbarGroupDOM = rootElement.ownerDocument.createElement("div");
    for (let i = 0; i < toolsbar_groups.length; i++) {
        let groups = toolsbar_groups[i];
        if (groups.length > 0) {
                ...
                if (tool.type == "select") {
                    if (tool.optionName) {
                        ...
                        if (options && options.length) {
                            ...
                            selecDivDOM.insertAdjacentElement("beforeend", selectDOM);
                            selecDivDOM.setAttribute('commendName', tool.commendName)
                            selectDOM.onchange = (e) => {
                                rootElement.DCExecuteCommand(tool.commendName, false, e.target.value);
                            };
                    ...
                    toolDivDOM.classList.add("toolbar-box");
                    toolDivDOM.insertAdjacentElement("beforeend", svgDOM);
                    if (tool.commendName && tool.commendName != '') {
                        toolDivDOM.setAttribute('commendName', tool.commendName)
                    }
                    toolDivDOM.removeEventListener("click", null);
                    toolDivDOM.onclick = (e) => {
                        if (tool.onclick) {
...
let buildColorPicker = function (e, commendName, rootElement) {
    showMaskBg(rootElement);
    e.stopPropagation();
    var closest = jQuery(e.target).closest(".toolbar-box");
    let colorParentDiv = rootElement.querySelector(".DC-renderColorPickerBox");
    let oldflag = true;
    if (!colorParentDiv) {
        oldflag = false;
    ...
    var RecentlyUsedDiv = document.querySelectorAll("#recentlyUsedDom>.dc_toolbar_color_box");
    ...
    dcColorPicker.onclick = function (e) {
        e.stopPropagation();
        if (e.target.classList.contains("dc_toolbar_color_box")) {
...
let changeCurrentProp = function (rootElement) {
    if (rootElement) {
        for (let i = 0; i < toolsbar_groups.length; i++) {
            let groups = toolsbar_groups[i];
            if (groups.length > 0) {
                for (let j = 0; j < groups.length; j++) {
                    ...
                    let itemDOM = rootElement.querySelector(`div[commendname='${tool.commendName}']`)
                    if (itemDOM) {
                        if (tool.commendName == 'fontname') {
                            let selectDOM = itemDOM.querySelector('select')
                            selectDOM.value = rootElement.CurrentFontName;
                        } else if (tool.commendName == 'fontsize') {
                            let selectDOM = itemDOM.querySelector('select')
                            selectDOM.value = rootElement.CurrentFontSize;
                        }else if (tool.commendName == 'color' || tool.commendName == 'backcolor') {
                            
                        } else {
                            var CommandInfo = rootElement.GetCommandStatus(tool.commendName);
                            if (CommandInfo.Supported == false) {
                                // 系统不支持命令
                                itemDOM.style.backgroundColor = "red";
                                itemDOM.disabled = true;
                            }
                            else if (CommandInfo.Enabled == false) {
                                // 命令当前无效
                                //uiElement.style.backgroundColor = "lightgray";
                                itemDOM.style.color = "#999999";
                                itemDOM.disabled = true;
                            }
                            else {
                                itemDOM.style.color = "";
                                itemDOM.disabled = false;
                            }
                            if (CommandInfo.Checked == true) {
                                // 命令处于勾选状态
                                //itemDOM.style.border = "1px solid black";
                                itemDOM.style.borderRadius = "5px";
                                itemDOM.style.backgroundColor = "rgb(228, 228, 228)";
                            }
                            else {
                                itemDOM.style.borderRadius = "";
...
let MenuOptionList = {
    ...
    insertMenu: [
        ...
        },
        {
            ...
            onClick: (rootElement) => {
                var options = {
                    Name: "rad" + Math.floor(Math.random() * 90000000 + 10000000), //单选框的Name属性相同
        ...
        },
        {
            ...
            onClick: (rootElement) => {
                var options = {
                    Name: "chk" + Math.floor(Math.random() * 90000000 + 10000000), //单选框的Name属性相同
...
let colorPickerHTML = `
        ...
        <div class="dc_inner_color_picker_default dc_toolbar_color_box">默认</div>
        <div class="dc_inner_color_picker_choose">
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #ffffff;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f2f0f3;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #dad8db;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #bfbdc0;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #a5a3a6;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #959396;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #010002;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #7f7d80;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #5b595c;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #3f3d40;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #272527;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #09070a;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #4b5265;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f5f3f6;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #cac7cf;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #86889e;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #262427;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #212024;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #3f71fc;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #e6effb;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #cadbfd;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #a0befc;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #2253ba;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #182f76;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #3ca4f3;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #e4faf8;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #d0edff;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #a5dcfe;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #2d74a0;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #18405b;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #469959;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #edf8f0;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #cbe5d4;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #a2d6b4;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #337c4f;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #27472f;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #d63733;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #fce7ec;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f9c9c9;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f6979a;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #941d15;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #4c110b;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #e98209;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #faf3eb;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f9dcc3;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f6b785;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #ae6203;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #5b2d0c;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #efc500;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f8f9e0;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #ffefa9;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #fce261;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #998208;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #5c5006;"></div>
            </div>
            <div class="dc_color-group">
                <div class="dc_toolbar_color_box" style="background-color: #9631dd;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #fbeafb;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #edc9fd;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #cd8eff;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #54288f;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #371347;"></div>
            </div>
        ...
        <div class="dc_inner_color_picker_standard">
            ...
            <div class="dc_inner_color_picker_standard_choose">
                <div class="dc_toolbar_color_box" style="background-color: #af0308;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f10300;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #f7c003;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #fffb00;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #9cd04a;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #37b246;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #39b1f7;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #2372b6;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #0d1d63;"></div>
                <div class="dc_toolbar_color_box" style="background-color: #6a319b;"></div>
            </div>
        </div>
        <div class="dc_inner_color_picker_standard">
...
let colorPickerSTYLE = ` .dc_inner_color_picker {
        ...
        .dc_inner_color_picker_default.dc_toolbar_color_box {
            width: 100%;
            height: 26px;

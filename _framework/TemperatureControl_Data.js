...
export let TemperatureControl_Data = {
    ...
    NewYAxis: function () {
        ...
        //20250326DUWRITER5_0-4277 lxy新增
        yaxis.MinValueVisible = true; //在标尺中，展示最小的刻度值,默认展示true
        yaxis.MaxValueVisible = true; //在标尺中，展示最大刻度值，默认展示true
        yaxis.FahrenheitVisible = false; //当体温为摄氏度时，是否显示华氏度刻度表，默认false
        yaxis.CorrectTheVerticalOfTickTextMaxMin = true; //修正最大最小值的垂直位置，解决最大值和最小值由于防止超出边界而被挤压的情况.默认true修正。当设置为false时，最大值和最小值会以自身位置居中，可能会超出边界。
        yaxis.ShowInRule = true; //新增：是否将当前Y轴展示在左侧标尺中，默认true,为false时，在左侧Y轴不绘制
        yaxis.RightPadding = 0; //右边距,右对齐时，刻度距离右边框的距离，TickTextAlignment=Far时有效
        yaxis.LeftPadding = 0; //左边距,左对齐时，刻度距离左边框的距离，TickTextAlignment=Near时有效
        return yaxis;
    },
    /**
     * 新建时间轴数据点对象
    ...
    NewValuePoint: function () {
        ...
        var point = {
            ...
            LanternTitleText: null,      //灯笼标题文本                                       //适配
            ...
            UpAndDown: "None",                                                                  //适配
            //////////[DUWRITER5_0-4151] 20250212 lxy新增复测标记
            EnableRetestMarkValue: false, //是否显示复测标记                                      //适配
            RetestMarkValue: null, //复测值                                                         //适配
            CharRetestMark: "V",//复测字符                                                       //适配
            CharRetestMarkColor: null, //复测颜色                                               //适配
            CharRetestMarkAlign: 'center', //测量标记对齐方式"Left" "Right" "Center(默认)"       //适配
            CharRetestMarkFontSize: 10, //测量标记字体大小                                              //适配
            ShowTip: true,                          //悬浮提示是否展示              //适配
        };
        return point;
    },
    /**
    ...
    NewTitleLine: function () {
        ...
        line.TitleAlign = "Center";//数据行标题对齐方式"Near""Far""Center" "Justify两端对齐" "SpaceAround:两端空白"                   //适配
        ...
        line.TitleFontBold = false; //标题加粗                                            //适配
    ...
    NewTextLabel: function () {
        ...
        //20250326DUWRITER5_0-4277新增
        label.SymbolVisible = false;//新增图标显隐
        label.SymbolStyle = "";//新增图标样式
        label.SymbolColorValue = null;//新增图标颜色
        label.SymbolSize = 14;//新增图标大小
        label.SymbolPosition = "bottom";//新增图标位置 默认为"bottom正下方"，"left左边"
        //[DUWRITER5_0-4361] 20250417 lxy新增
        label.Name = ""; //文本名称
        return label;
    },
    /**
    ...
    NewHeaderLabel: function () {
        ...
        label.FontBold = false;       //数值下划线     20250326DUWRITER5_0-4277新增           
        return label;
    },
    /**
     * 新建空白的时间轴文档设置信息对象
    ...
    NewTemperatureDocumentConfig: function () {
        ...
        config.DateFormatString = "dd";     //时间字符串格式
        ...
        config.DateFormatStringForFirstIndexOtherPage = "yyyy-MM-dd";   //非首页首个日期时间字符串格式
        ...
        // 20250326DUWRITER5_0-4277新增
        config.SpecifyRightPlaceholderElementWidth = 16; //新增，右侧占位区域宽度。只有在 TemperatureMode=="Fahrenheit"时有效。
        config.TemperatureMode = "Defalut"; //新增，默认值为Defalut，带有华氏度的体温单为：Fahrenheit
        config.RightPlaceholderElement = false;//新增，是否显示右侧占位区域。只有在 TemperatureMode=="Fahrenheit"时有效。
...
export let TemperatureControl_XMLToJSON = {
    ...
    parseXmlData: function (node) {
        ...
        if (children.length > 0) {
            ...
            if (TemperatureControl_XMLToJSON.toArrLabelsName.includes(pNodeName)) {
                ...
                //[DUWRITER5_0-4512] 202506099 lxy处理文本标签字体颜色值无法解析的情况
                var isLabel = (pNodeName == 'Labels');
                return TemperatureControl_XMLToJSON.transAttributesAndChildToObject(children, defaultValue, isLabel);
            } else {
    ...
    transAttributesAndChildToObject: function (children, defaultValue = {}, isLabel = false) {
        let result = [];
        for (let i = 0; i < children.length; i++) {
            ...
            attributes.forEach(item => {
                ...
                //[DUWRITER5_0-4512] 202506099 lxy处理文本标签字体颜色值无法解析的情况
                if (item.name == 'ColorValue' && isLabel) {
                    obj['TextColorValue'] = TemperatureControl_XMLToJSON.transType(item.nodeValue, item.name);
                }
            });
            //处理子元素

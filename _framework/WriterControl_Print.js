    ...
    */
    SaveLocalPDF: function (options) {// rootElement, strFileName, callBack, documentsXml, forceBlack = false) {
        ...
        function GenPDFFile() {
            ...
            resultData = DCTools20221228.GetResultBinaryFullData(
                rootElement.__DCWriterReference.invokeMethod("SaveAsLocalPDF", taskResultValues, bolForceBlack, bolForOFD));
            if (resultData != null) {
                            ...
                            if (this.status == 200 || this.status == 304) {
                                ...
                                if (blob == null || blob.size < 100) {
                                    ...
                                    if (typeof rootElement.EventOfPDFFontDownloadError == "function") {
                                        rootElement.EventOfPDFFontDownloadError(window.__DCSR.FailToDownLoadFontFileForPDFOFD + strUrl)
                                    }
                                }
                                else {
                                ...
                                console.warn(window.__DCSR.FailToDownLoadFontFileForPDFOFD + "code:[" + this.status + "]"+ strUrl);
                                rootElement.__DCWriterReference.invokeMethod("CancelSaveAsLocalPDF");
                                if (typeof rootElement.EventOfPDFFontDownloadError == "function") {
                                    rootElement.EventOfPDFFontDownloadError(window.__DCSR.FailToDownLoadFontFileForPDFOFD + strUrl)
                                }
    ...
    PrintPreview: function (containerID, options, eleString) {
        ...
        // 修复打印预览控件预览文档时页面容器显示占位的问题【DUWRITER5_0-4728】
        // 因为不确定隐藏编辑视图前页面容器是否存在，所以需要确保页面容器存在
        WriterControl_UI.GetPageContainer(rootElement);
        // 隐藏编辑视图
        ...
        if (eleString) {
            ...
            var strCode = rootElement.__DCWriterReference.invokeMethod("GetPageIndexWidthHeightForPrint",
                true,
                options,
                false);
            ...
            for (var iCount = 0; iCount < datas.length; iCount++) {
                ...
                element.onclick = function (eve) {
                    ...
                    var strPageList = rootElement.__DCWriterReference.invokeMethod("WASMPrintPreviewHandleMouseClick",
    ...
    InnerDrawOnePage: function (element, bolPrintPreview, oldCtl) {
        ...
        //if (element.nodeName.toLowerCase() == "svg") {
        // 采用SVG模式进行打印预览
        var strSVG = DCTools20221228.GetResultUTF8String(
            rootElement.__DCWriterReference.invokeMethod("PaintPageForPrintUseSVG",
                element.PageIndex,
                bolPrintPreview,
                true));
        if (strSVG != null) {
        ...
        var printFun = function () {
            ...
            if (rootElement.IsWriterPrintPreviewControlForWASM && options && options.NotPrintWatermark == true) {
                ...
                for (var i = 0; i < iframeDoc_svg.length; i++) {
                    ...
                    // 获取SVG元素的第二个子元素 //wyc20250911根据新结构调整写法DUWRITER5_0-4795
                    var watermarkEle = svg.querySelector("[iswatermark='true']");
                    if (watermarkEle != null && watermarkEle.remove) {
                        // 删除第二个子元素
                        watermarkEle.remove();
                    }
                }
     ...
     * 在此处添加蒙版用于控制选中打印效果
     */
    PrintSelectionMask: function (rootElement, iframeDoc) {
        // console.log('PrintSelectionMask', iframeDoc);
        var selection = rootElement.ownerDocument.getSelection();

        if (rootElement.PrintRemoveChildEle == null) {
            //找到对应的子元素
            rootElement.PrintRemoveChildEle = (parentEle, removeEle, selection) => {
                if (rootElement.PrintRemoveChildEle.baseNode != null && rootElement.PrintRemoveChildEle.focusNode != null) {
                    removeEle.innerHTML = "";
                    return;
                }

                var allChild = parentEle.children;
                var removeChild = removeEle.children;
                allChild = Array.prototype.slice.call(allChild);
                
                allChild.forEach((ele, index) => {
                    if (ele.nodeName == 'g') {
                        //进入下一层
                        rootElement.PrintRemoveChildEle(ele, removeChild[index], selection);
                        
                    } else {
                        if (rootElement.PrintRemoveChildEle.baseNode == null) {
                            //开始
                            if (ele == selection.baseNode.parentNode || ele == selection.baseNode) {
                                rootElement.PrintRemoveChildEle.baseNode = ele;
                                //获取元素宽度
                                var textWidth = ele.getBBox().width;
                                //获取文本的坐标
                                var textSplit = ele.getAttribute("x").split(" ");
                                var hasTextLength = ele.getAttribute("textLength");
                                if (textSplit.length == 1 && hasTextLength) {
                                    var cutText = ele.textContent.substring(0, selection.baseOffset);
                                    var font = `font-family:${window.getComputedStyle(ele).fontFamily
                                        };font-size:${window.getComputedStyle(ele).fontSize}`;
                                    var cutWidth = getTextWidth(cutText,font)
                                    var percentage = (parseFloat(cutWidth) / parseFloat(hasTextLength) * 100 + "%");
                                } else {
                                    var percentage = (parseFloat(textSplit[selection.baseOffset]) - parseFloat(textSplit[0])) / textWidth * 100 + "%";
                                }
                                
                                removeChild[index].style.clipPath = `inset(0 0 0 ${percentage})`;
                                return
                            } else {
                                if (removeChild[index].nodeName != "text") {
                                    if (removeChild[index].nodeName == "image" || removeChild[index].nodeName == 'use') {
                                        removeChild[index].style.opacity = "0";
                                    } else if (removeChild[index].nodeName == "line") {
                                        removeChild[index].setAttribute("stroke", "transparent");
                                    } else {
                                        removeChild[index].setAttribute("fill", "transparent");
                                    }
                                } else {
                                    removeChild[index].innerHTML = "";
                                }
                                return
                            }
                        } else {
                            if (rootElement.PrintRemoveChildEle.focusNode == null) {
                                if (ele == selection.focusNode.parentNode || ele == selection.focusNode) {
                                    rootElement.PrintRemoveChildEle.focusNode = ele;
                                    //获取元素宽度
                                    var textWidth = ele.getBBox().width;
                                    //获取文本的坐标
                                    var textSplit = ele.getAttribute("x").split(" ");
                                    var hasTextLength = ele.getAttribute("textLength");
                                    //获取到字体大小
                                    var textFontSize = ele.getBBox().height;
                                    if (textSplit.length == 1 && hasTextLength) {
                                        var cutText = ele.textContent.substring(0, selection.focusOffset);
                                        var font = `font-family:${window.getComputedStyle(ele).fontFamily
                                            };font-size:${window.getComputedStyle(ele).fontSize}`;
                                        var cutWidth = getTextWidth(cutText, font)
                                        var percentage = ((parseFloat(hasTextLength) - parseFloat(cutWidth)) / parseFloat(hasTextLength) * 100 + "%");
                                    } else {
                                        var targetWidth = selection.focusOffset >= textSplit.length ? parseFloat(textSplit[textSplit.length - 1]) + textFontSize : parseFloat(textSplit[selection.focusOffset]);
                                        var percentage = ((parseFloat(textSplit[textSplit.length - 1]) + textFontSize) - targetWidth) / textWidth * 100 + "%";
                                    }

                                    removeChild[index].style.clipPath = `inset(0 ${percentage} 0 0)`;
                                    return
                                } else {
                                    if (removeChild[index].nodeName == 'use') {
                                        //判断是否存在
                                        removeChild[index].setAttribute("opacity", "1");
                                    } else if (removeChild[index].nodeName == "line") {
                                        removeChild[index].setAttribute("stroke", "transparent");
                                    }
                                }
            ...
            function getTextWidth(text, font) {
                ...
                element.style.cssText = font; // 设置字体样式
            ...
            rootElement.PrintRemoveChildEle.baseNode = null;
            rootElement.PrintRemoveChildEle.focusNode = null;
        }

        //在此处修改正反选
        var changeSelect = {};
        if (selection.focusNode == selection.baseNode) {
            if (selection.focusOffset > selection.baseOffset) {
                ...
                changeSelect = {
                    focusNode: selection.baseNode,
                    focusOffset: selection.baseOffset,
                    baseNode: selection.focusNode,
                    baseOffset: selection.focusOffset
                }
            }
        } else {
            //判断位置
            var focusEle = selection.focusNode;
            focusEle = focusEle.nodeName == "#text" ? focusEle.parentNode : focusEle;
            var baseEle = selection.baseNode;
            baseEle = baseEle.nodeName == "#text" ? baseEle.parentNode : baseEle;
            //判断是否在同一个svg中
            var focusSvg = focusEle.viewportElement;
            var baseSvg = baseEle.viewportElement;
            //同一页中
            if (focusSvg == baseSvg) {
                var focusObj = focusEle.getBBox();
                var baseObj = baseEle.getBBox();
                if (focusEle == baseEle) {
                    if (focusObj.x < baseObj.x) {
                        changeSelect = {
                            focusNode: selection.baseNode,
                            focusOffset: selection.baseOffset,
                            baseNode: selection.focusNode,
                ...
                //不同页
                var focusObj = focusEle.getBoundingClientRect();
                var baseObj = baseEle.getBoundingClientRect();
                if (focusObj.y < baseObj.y) {
                    changeSelect = {
                        focusNode: selection.baseNode,
                        focusOffset: selection.baseOffset,
        ...
        changeSelect.type = selection.type;
        selection = changeSelect;

        //假设iframe中的dom结构和printdiv中的结构完全一致
        var printpreviewEle = rootElement.querySelector("[dctype=page-printpreview]");
        //找到所有的svg
        var printpreviewSvg = printpreviewEle.querySelectorAll(":scope>svg");

        //找到打印用的dom结构
        var printSvg = iframeDoc.body.querySelector("div").querySelectorAll(":scope>svg");
        //循环找到所有的文本
        printpreviewSvg = Array.prototype.slice.call(printpreviewSvg);
        printpreviewSvg.forEach((item, index) => {
            if (item.innerHTML != '<text x="50" y="50">请等待...</text>') {
                if (rootElement.PrintRemoveChildEle.focusNode != null) {
                    printSvg[index].remove();
                } else {
                    //找打目标元素
                    var printPreviewHeader = item.querySelector("#divXTextDocumentHeaderElement");
                    var printHeader = printSvg[index].querySelector("#divXTextDocumentHeaderElement");
                    if (printPreviewHeader) {
                        rootElement.PrintRemoveChildEle(printPreviewHeader, printHeader, selection);
                    }
                    var printPreviewBody = item.querySelector("#divDocumentBody_0");
                    var printBody = printSvg[index].querySelector("#divDocumentBody_0");
                    if (printPreviewBody) {
                        rootElement.PrintRemoveChildEle(printPreviewBody, printBody, selection);
                    }
                    var printPreviewFooter = item.querySelector("#divXTextDocumentFooterElement");
                    var printFooter = printSvg[index].querySelector("#divXTextDocumentFooterElement");
                    if (printPreviewFooter) {
                        rootElement.PrintRemoveChildEle(printPreviewFooter, printFooter, selection);
                    }
                    //新增判断
                    if (printPreviewHeader == null && printPreviewBody == null && printPreviewFooter == null) {
                        //直接找clipPath节点
                        var allClipPath = item.querySelectorAll("clippath");
                        var allPrintClipPath = printSvg[index].querySelectorAll("clippath");
                        if (allClipPath.length > 0) {
                            allClipPath = Array.prototype.slice.call(allClipPath);
                            allClipPath.forEach((clippath, eleIndex) => {
                                rootElement.PrintRemoveChildEle(clippath.nextElementSibling, allPrintClipPath[eleIndex].nextElementSibling, selection);
                            })
                        }
                    }
                }
    ...
    */
    TemplatePrintingWithCells: function (doc, hiddenCellData, EnableHide) {
        ...
        WriterControl_Print.walkThroughPreviewDoc(doc, function (pageId, pageDOM) {
            ...
            // 处理零宽字符，包括常见的零宽空格（U+200B）、零宽非连接符（U+200C）、零宽连接符（U+200D）、U+2002等
            var txt = tdDOM.innerText.replace(/[\u2002\u200B\u200C\u200D\uFEFF]/g, ""); // 替换所有8194空格、零宽空格、零宽非连接符、零宽连接符、BOM
            // 修复存在图片套打的情况【DUWRITER5_0-4768】
            var imgNode = tdDOM.querySelector("img");
            // 判断单元格内容是否需要打印：有非零宽字符的文本，或有图片
            // 这里判断文本去除零宽字符后是否还有内容
            if ((txt && txt.trim().length > 0) || imgNode) {

...
export let WriterControl_DrawFu = {
    ...
    CreateDefaultData: function (rootElement, data) {
        ...
        var DefaultData = {
            ...
            NumOfPages: 0,
            Config: {
                "SpecifyRightPlaceholderElementWidth": 16, //右侧占位区域宽度
        ...
        var CalculationData = {
            ...
            TemperatureMode: "Defalut", //体温单展示模式 Fahrenheit:带有刻度和右侧占位且隐藏左侧边框的体温单，default：默认模式的体温单
            UIDElePositon: [], //元素位置
        ...
        //记录体温单模式 Fahrenheit:带有刻度和右侧占位且隐藏左侧边框的体温单，default：默认模式的体温单
        if (data.Config && data.Config.TemperatureMode && data.Config.TemperatureMode == "Fahrenheit") {
            WriterControl_DrawFu.DocumentOptions.CalculationData['TemperatureMode'] = "Fahrenheit";
        } else {
            WriterControl_DrawFu.DocumentOptions.CalculationData['TemperatureMode'] = "Defalut";
        }
    ...
    CreatePageContainer: function (rootElement) {
        var hasPageContainer = rootElement.querySelector("[dctype=page-container]");
        if (!hasPageContainer) {
            ...
            var pageContainer = hasPageContainer;
    ...
    ChangeData: function (rootElement, data, type) {
        ...
        if (data && typeof data == "object" && data.Config) {
            for (var i in data.Config) {
                if (i == "PageSettings") {
                    ...
                    if (data.Config[i] || data.Config[i] === false) {
        ...
        for (var i = 0; i < defaultData.HeaderLines.length; i++) {
            ...
            if (typeof thisHeaderLine.Title !== "string") {
                thisHeaderLine.Title = '';
            }
            var thisTitle = thisHeaderLine.Title;
            //如果带有指定页码修改的标题PageTitleTexts，则拿到一个最长的标题计算宽度
            if (thisHeaderLine.PageTitleTexts && thisHeaderLine.PageTitleTexts.length > 0 && typeof thisHeaderLine.PageTitleTexts === "string") {
                var PageTitleTextsArray = thisHeaderLine.PageTitleTexts.split(",") || [];
                if (PageTitleTextsArray && PageTitleTextsArray.length) {
                    PageTitleTextsArray.forEach(item => {
                        if (item.length > thisTitle.length) {
                            thisTitle = item;
                        }
                    });
                }
            }
            //如果带有指定时间修改的标题，则拿到一个最长的标题计算宽度
            if (thisHeaderLine.Name && defaultData.HeaderLinesChangeDate && defaultData.HeaderLinesChangeDate[thisHeaderLine.Name]) {
                let TitleValueChangeData = defaultData.HeaderLinesChangeDate[thisHeaderLine.Name].ValueChangeData || [];
                if (TitleValueChangeData && TitleValueChangeData.length) {
                    TitleValueChangeData.forEach(item => {
                        if (item.Value.length > thisTitle.length) {
                            thisTitle = item.Value;
                        }
                    });
                }
            }
        ...
        for (var i = 0; i < defaultData.FooterLines.length; i++) {
            ...
            if (typeof thisFooterLine.Title !== 'string') {
                thisFooterLine.Title = "";
            }
            var thisTitle = thisFooterLine.Title;
            //如果带有指定页码修改的标题PageTitleTexts，则拿到一个最长的标题计算宽度
            if (thisFooterLine.PageTitleTexts && thisFooterLine.PageTitleTexts.length > 0 && typeof thisFooterLine.PageTitleTexts === "string") {
                var PageTitleTextsArray = thisFooterLine.PageTitleTexts.split(",") || [];
                if (PageTitleTextsArray && PageTitleTextsArray.length) {
                    PageTitleTextsArray.forEach(item => {
                        if (item.length > thisTitle.length) {
                            thisTitle = item;
            ...
            //如果带有指定修改的标题，则拿到一个最长的标题计算宽度
            if (thisFooterLine.Name && defaultData.FooterLinesChangeDate && defaultData.FooterLinesChangeDate[thisFooterLine.Name]) {
                let TitleValueChangeData = defaultData.FooterLinesChangeDate[thisFooterLine.Name].ValueChangeData || [];
                if (TitleValueChangeData && TitleValueChangeData.length) {
                    TitleValueChangeData.forEach(item => {
                        if (item.Value.length > thisTitle.length) {
                            thisTitle = item.Value;
        ...
        for (var i = 0; i < defaultData.YAxisInfos.length; i++) {
            ...
            var thisTitle = thisYAxisInfos.Title ? thisYAxisInfos.Title : "体征项目";
            var thisWidth = (thisTitle.length) * calculationData.singleLableTextWidth;
            //判断y轴的值
            ...
            //[DUWRITER5_0-4307]20250401 lxy 解决Y轴最小值是0时,不展示的问题
            for (var j = 0; j <= thisYAxisInfos.YSplitNum; j++) {
                if (calculationData.YAxisInfosData[index] == null) {
                    calculationData.YAxisInfosData[index] = [[], thisYAxisInfos];
                }
                calculationData.YAxisInfosData[index][0].unshift(parseFloat(((diff * j) + min).toFixed(2)));
            }
            ...
            // if (!calculationData.TemperatureMode || calculationData.TemperatureMode !== "Fahrenheit") {
            //     //[DUWRITER5_0-3598] 20240920 lxy 解决Y轴文字首次加载位置不准确问题 
            //     if ((thisYAxisInfos.TitleVisible === false || thisYAxisInfos.TitleVisible === "false") && calculationData.DCType == "DCTemperatureControlForWASM") {
            //         calculationData.YAxisInfosData[index] = [[], thisYAxisInfos];
            //         calculationData.YAxisInfosData["dc_" + thisYAxisInfos.Name].Index = index;
            //         continue;
            //     }
        ...
        //[DUWRITER5_0-4369] 20250425 lxy 解决Y轴绘制时宽度计算不准确的问题（第一遍循环,是为了筛选出所有被指定为阴影的Y轴，第二遍是为了准确计算Y的宽度）
        calculationData.YAxisInfosSingleWidth = [];
        for (var i = 0; i < defaultData.YAxisInfos.length; i++) {
            var thisYAxisInfos = defaultData.YAxisInfos[i];
            //阴影用的Y轴不展示
            if (calculationData.ShadowNameArr.Name) {
            ...
            //隐藏的Y轴不展示
            if (thisYAxisInfos.ShowInRule === false && calculationData.DCType == "DCTemperatureControlForWASM") {
                continue;
            }
            ...
            //默认值宽度24* 1.31
            var currentWidth = (24 * 1.31);
            //当存在title时需要计算文本的宽度
            if (thisYAxisInfos.Title) {
                var titleWidth = calculationData.singleLableTextWidth * thisYAxisInfos.Title.length;
                currentWidth = currentWidth > titleWidth ? currentWidth : titleWidth;
            }
            if (thisYAxisInfos.BottomTitle) {
                var bottomTitleWidth = calculationData.singleLableTextWidth * thisYAxisInfos.BottomTitle.length;
                currentWidth = currentWidth > bottomTitleWidth ? currentWidth : bottomTitleWidth;
            }

            //判断是否存在指定宽度
            if (thisYAxisInfos.SpecifyTitleWidth && parseFloat(thisYAxisInfos.SpecifyTitleWidth) > 0) {
                currentWidth = parseFloat((thisYAxisInfos.SpecifyTitleWidth / 300 * 96.00001209449).toFixed(2));
            }

            // 当体温单模板为指定模式时，判断Y轴的体温是否需要显示华氏度，如果需要就把宽度放大
            if (calculationData.TemperatureMode && calculationData.TemperatureMode.toLowerCase() == "fahrenheit") {
                if (thisYAxisInfos.FahrenheitVisible) {
                    currentWidth *= 2;
                }
            }
            //当没有合并到左侧时，才计算宽度
            if (!thisYAxisInfos.MergeIntoLeft || thisYAxisInfos.MergeIntoLeft == "false") {
                calculationData.YAxisInfosSingleWidth.push(currentWidth);
            }
        }
        //计算总宽度
        calculationData.YAxisInfosTotalWidth = 0;
        if (calculationData.YAxisInfosSingleWidth && calculationData.YAxisInfosSingleWidth.length) {
            for (var i = 0; i < calculationData.YAxisInfosSingleWidth.length; i++) {
                calculationData.YAxisInfosTotalWidth += (calculationData.YAxisInfosSingleWidth[i] || 0);
            }
        }
                    ...
                    for (var j = 0; j < thisValue.Datas.length; j++) {
                        ...
                        if (keywordLength && Object.keys(keywordLength).length > 0 && thisData.Text) {
                            for (var k in keywordLength) {
                                ...
                                //[DUWRITER5_0-4435] 20250513 lxy 解决多个关键字匹配问题
                                var kArr = k.split(",");
                                for (var kk = 0; kk < kArr.length; kk++) {
                                    var kItem = kArr[kk];
                                    if (thisData.Text.indexOf(kItem) >= 0) {
        ...
        while (startDate <= endDate) {
            for (var z = 0; z < calculationData.TickValuesData.length; z++) {
                ...
                if (z == 0) {
                    ...
                    //周一的日期格式
                    var newWeek = thisDate.getDay();


                    //[DUWRITER5_0-4168] 20250212 增加一个属性控制日期数据补0
                    var TitleLineSerialDateCompletion = rootElement.getAttribute('TitleLineSerialDateCompletion') || null;
                    if (TitleLineSerialDateCompletion === 'true' || TitleLineSerialDateCompletion === true) {
                        if (newMonth < 10) {
                            newMonth = "0" + newMonth;
                        }
                        if (newDay < 10) {
                    ...
                    //[DUWRITER5_0-4298] 20250331lxy 修复日期格式化问题
                    var crossType = "";   //日期格式
                    if (calculationData.AllDateData.length == 0) {
                        //首页第一个日期格式
                        var crossType = defaultData.DateFormatStringForFirstIndexFirstPage;
                        if (!crossType) {
                            crossType = "yyyy-MM-dd";
                        }
                    } else {
                        if (thisYear != newYear) {
                            //判断跨年DateFormatStringForCrossYear
                            var crossType = defaultData.DateFormatStringForCrossYear;
                            if (!crossType) {
                                crossType = "yyyy-MM-dd";
                            }

                        } else if (thisMount != newMonth) {
                            //判断跨月DateFormatStringForCrossMonth
                            var crossType = defaultData.DateFormatStringForCrossMonth;
                            if (!crossType) {
                                crossType = "MM-dd";
                            }
                        } else {
                            if (calculationData.AllDateData.length % calculationData.NumOfDaysInOnePage == 0) {
                                //其他页第一个日期格式defaultData.DateFormatStringForFirstIndexOtherPage
                                var crossType = defaultData.DateFormatStringForFirstIndexOtherPage;
                            } else if (newWeek === 1) {
                                //每个周一，跨周defaultData.DateFormatStringForCrossWeek
                                var crossType = defaultData.DateFormatStringForCrossWeek;
                            } else {
                                //跨天defaultData.DateFormatString
                                var crossType = defaultData.DateFormatString;
                            }
                            if (!crossType) {
                                crossType = "dd";
                            }
                    ...
                    var crossTypeData = crossType.replace("yyyy", newYear);
                    crossTypeData = crossTypeData.replace("MM", newMonth);
                    crossTypeData = crossTypeData.replace("dd", newDay);
                    calculationData.AllDateData.push(String(crossTypeData));
                    thisMount = newMonth;
                    thisYear = newYear;
                } else {
                ...
                if (thisValue && thisValue.Datas) {
                    ...
                    for (var j = 0; j < calculationData.TotalPageNumber; j++) {
                        ...
                        for (var k = 0; k < thisValue.Datas.length; k++) {
                            ...
                            //[DUWRITER5_0-4603] 20250708 解决特殊项目数值为0时，不展示的问题
                            var hasText = thisData.Text ? thisData.Text : (thisData.Value == -10000 || (!thisData.Value && thisData.Value !== 0)) ? "" : String(thisData.Value);
                            if (hasText == "") {
                                continue;
                            }
    ...
    DrawSvg: function (rootElement) {
        ...
        titleSpan.style.fontFamily = "宋体,国标宋体";
        ...
        //判断是否存在zoomrate
        var zoomrate = rootElement.getAttribute("zoomrate");
        calculationData.ZoomRate = parseFloat(zoomrate) || 1;
        ...
        for (var i = 0; i < calculationData.TotalPageNumber; i++) {
            ...
            //绘制文本
            tooltip.append('text')
                .attr('style', `font-family:宋体,国标宋体;font-size:9pt;line-height: 1`)
            ...
            let resetSvg = WriterControl_DrawFu.DrawHeaderFooterLine(rootElement, svg, calculationData, defaultData, svgEle, i, "FooterLines");
            if (resetSvg) {
                break;
            }
            ...
            //绘制占位元素
            WriterControl_DrawFu.DrawBlockElements(svg, calculationData, defaultData, svgEle, i);
    ...
    DrawTitle: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        if (defaultData.Title && defaultData.Title.length > 0) {
            dcTitleG.append('text')
                .attr('style', `font-family:${defaultData.BigTitleFontName || defaultData.FontName || '宋体,国标宋体'} ;font-weight:${defaultData.BigTitleFontBold ? 900 : 0}; font-size:${defaultData.BigTitleFontSize}pt;text-anchor: middle;line-height: 1`)
                .attr('dctype', 'dcTitle')
                .attr('x', () => {
                    //带有刻度的体温单模式，需要增加右侧填充元素的宽度
                    if (calculationData.TemperatureMode && calculationData.TemperatureMode.toLowerCase() == "fahrenheit") {
                        var SpecifyRightPlaceholderElementWidth = defaultData.SpecifyRightPlaceholderElementWidth || 16;
                        SpecifyRightPlaceholderElementWidth = Math.round(parseFloat(SpecifyRightPlaceholderElementWidth) / 25.4 * 96);
                        return (SpecifyRightPlaceholderElementWidth + calculationData.InnerWidth) / 2;
                    }
                    return calculationData.InnerWidth / 2;
                })
    ...
    DrawPageNumber: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        var pageNumberEle = dcPageNumberG.append('text')
            .attr('style', `font-family:${defaultData.FontName || '宋体,国标宋体'} ;font-size:${defaultData.FontSize}pt;text-anchor: middle;line-height: 1.5`)
    ...
    DrawFooterDescription: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        dcFooterDescriptionG.append('text')
            .attr('style', `font-family:${defaultData.FontName || '宋体,国标宋体'} ;font-size:${defaultData.FontSize}pt;line-height: 1.5`)
    ...
    DrawLabel: function (svg, calculationData, defaultData, svgEle, i, parameters) {
        ...
        if (defaultData.Labels && defaultData.Labels.length > 0) {
            ...
            for (var j = 0; j < defaultData.Labels.length; j++) {
                ...
                //[DUWRITER5_0-4545]20250624 lxy 宽度兼容百分比
                const regex = /^\d+(\.\d+)?%$/;
                if (regex.test(thisLabel.Width)) {
                    var width = calculationData.InnerWidth * parseFloat(thisLabel.Width) / 100;
                } else {
                ...
                //是否为默认的体温单模式。 Fahrenheit:带有刻度和右侧占位且隐藏左侧边框的体温单，default：默认模式的体温单
                var ISDefaultTemperatureMode = !calculationData.TemperatureMode || calculationData.TemperatureMode !== "Fahrenheit";
                //为了绘制边框 疼痛强度在普通视图模式下才需要绘制边框
                if ((thisLabel.Text == "疼痛强度" || thisLabel.Text == "疼痛评分") && ISDefaultTemperatureMode) {
                ...
                if (thisLabel.ImageDataBase64String && thisLabel.ImageDataBase64String.length > 0) {
                    ...
                    var textEle = thisLabelG.append('text')
                        .attr('style', `font-family:${thisLabel.TextFontName || '宋体,国标宋体'} ;font-size:${thisLabel.TextFontSize || defaultData.FontSize}pt;font-weight:${thisLabel.TextFontBold ? 900 : 100};text-decoration: ${thisLabel.TextFontUnderline ? 'underline' : 'none'};`)
                        .attr('dctype', 'dcLables')
                    ...
                    } else if (thisLabel.Alignment == "Far") {
                    ...
                    if (thisLabel.LineAlignment == "Center") {
                        if (thisLabel.Text == "疼痛强度" || thisLabel.Text == "疼痛评分") {
                            textEle.attr('y', calculationData.PainScoreInfo.Top + (calculationData.PainScoreInfo.Height / 2) + (textHeight / 2));
                        } else {
                            textEle.attr('y', top + (height / 2) + (textHeight / 1.5 / 2));
                        }
                    } else if (thisLabel.LineAlignment == "Far") {
                        if (thisLabel.Text == "疼痛强度" || thisLabel.Text == "疼痛评分") {
                            textEle.attr('y', calculationData.PainScoreInfo.Top + (calculationData.PainScoreInfo.Height) - 2);
                        } else {
                            textEle.attr('y', top + height);
                        }
                    } else {
                        if (thisLabel.Text == "疼痛强度" || thisLabel.Text == "疼痛评分") {
                            textEle.attr('y', calculationData.PainScoreInfo.Top + textHeight);
                        } else {
                            textEle.attr('y', top + textHeight);
                        }
                    }
                    ...
                    if (thisLabel.SymbolVisible && thisLabel.SymbolStyle) {
                        // var SymbolSize = parseFloat(((thisLabel.SymbolSize || 5) / 300 * 96.00001209449).toFixed(2));
                        ...
                        var IconData = {
                            ...
                            x: () => {
                                ...
                                //是否指定位置，默认正下方
                                if (thisLabel.SymbolPosition && thisLabel.SymbolPosition !== '') {
                                    if (thisLabel.SymbolPosition.toLowerCase().trim() === 'left') {
                                        return textEleX - SymbolSize - 2;
                                    }
                                }
                            ...
                            y: () => {
                                ...
                                if (thisLabel.SymbolPosition && thisLabel.SymbolPosition !== '') {
                                    if (thisLabel.SymbolPosition.toLowerCase().trim() === 'left') {
                                        return textEleY + (textEleHeight / 2);
                                    }
                                }
            ...
            if (thisHeaderLables && thisHeaderLables.length > 0) {
                ...
                for (var k = 0; k < thisHeaderLables.length; k++) {
                    ...
                    thisHeaderLineG.append('text')
                        ...
                        .attr('style', 'font-size:' + (thisHeaderLables[k].FontSize || defaultData.FontSize || 9) + 'pt;font-weight:' + (thisHeaderLables[k].FontBold === 'true' || thisHeaderLables[k].FontBold === true ? 900 : 0) + ';font-family:' + (defaultData.FontName || '宋体,国标宋体') + ';line-height: 1')
                        .attr('x', dcHeadLineG.node().getBBox().width)
                        .attr('y', calculationData.LableHeight / 1.5)
                        .html(() => {
                            var titleValue = thisHeaderLables[k].Value;
                            //如果存在转科的情况
                            if (defaultData.HeaderLabelChangeDate && defaultData.HeaderLabelChangeDate[thisHeaderLables[k].UID]) {
                                var ValueChangeData = defaultData.HeaderLabelChangeDate[thisHeaderLables[k].UID].ValueChangeData || [];
                                var ValueConnectorSymbol = defaultData.HeaderLabelChangeDate[thisHeaderLables[k].UID].ValueConnectorSymbol || "→";
                                ...
                                for (var z = 0; z < ValueChangeData.length; z++) {
                                    ...
                                    if (changeTime >= thisBand[0] && changeTime <= thisBand[thisBand.length - 1]) {
                                        //当前页存在转科的情况
                                        titleValue += `${ValueConnectorSymbol}${ValueChangeData[z].Value}`;
                                    } else {
                                        if (changeTime < thisBand[0]) {
                                            titleValue = ValueChangeData[z].Value;
                                        }
                                    }
                                }
                ...
                var HeaderLabelWidth = calculationData.InnerWidth;
                //判断是否为带有刻度且右侧需要占位的
                if (calculationData.TemperatureMode && calculationData.TemperatureMode === "Fahrenheit") {
                    var blockWidth = defaultData.SpecifyRightPlaceholderElementWidth || 16;
                    blockWidth = Math.round(parseFloat(blockWidth) / 25.4 * 96);
                    HeaderLabelWidth += blockWidth;
                }
                var x = parseFloat((HeaderLabelWidth - thisWidth) / (thisHeaderLables.length - 1).toFixed(2));
                if (x < 0) {
                    //对当前数组进行重组
    ...
    DrawTableContainer: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        var dcTableContainerGRect = dcTableContainerG.append('rect')
            .attr('x', 0)
            .attr('y', 0)
        ...
        //体温单模式 Fahrenheit:带有刻度和右侧占位且隐藏左侧边框的体温单，default：默认模式的体温单
        if (calculationData.TemperatureMode !== "Fahrenheit") {
            dcTableContainerGRect
                .attr('stroke', "black")
    ...
    DrawHeaderFooterLine: function (rootElement, svg, calculationData, defaultData, svgEle, i, type) {
        ...
        //标题是否绘制边框
        var titleXStartLinePos = 0; //横线的开始位置

        //记录体温单模式 Fahrenheit:带有刻度和右侧占位且隐藏左侧边框的体温单，default：默认模式的体温单
        if (calculationData.TemperatureMode && calculationData.TemperatureMode === "Fahrenheit") {
            titleXStartLinePos = calculationData.FirstTextWidth;
        }
        ...
        for (var index = 0; index < lineData.length; index++) {
            ...
            //当前行标题加粗
            var titleFontBold = d.TitleFontBold === 'true' || d.TitleFontBold === true ? 900 : 100;
            ...
            //在此处对标题进行判断(根据指定页的标题判断，PageTitleTexts)
            var allPageTitle = (d.PageTitleTexts && d.PageTitleTexts.length && typeof d.PageTitleTexts === 'string') ? d.PageTitleTexts.split(",") : [];
            var currentTitle = (allPageTitle.length && allPageTitle[i]) || d.Title;
            if (typeof currentTitle !== 'string') {
                currentTitle = "";
            }

            //在此处对标题进行判断(根据时间范围的标题判断，HeaderLinesChangeDate FooterLinesChangeDate)
            //[DUWRITER5_0-4117]20250114 lxy 每页可以自定义标题功能
            if (d.Name !== '') {
                var linesChangeObject = (linesChangeDate && linesChangeDate[d.Name]) || null;
                if (linesChangeObject && linesChangeObject.ValueChangeData && linesChangeObject.ValueChangeData.length) {
                    //修改标题内容数组
                    var ValueChangeData = linesChangeObject.ValueChangeData || [];
                    for (var z = 0; z < ValueChangeData.length; z++) {
                        var changeTime = new Date(ValueChangeData[z].Time).getTime(); //修改时间
                        //如果修改时间大于thisBand[0],小于thisBand[thisBand.length-1],则进行修改。剩余的页数中使用最后一次修改的内容
                        if (changeTime <= thisBand[thisBand.length - 1]) {
                            currentTitle = ValueChangeData[z].Value;
                        }
                    }
                }
            }
            ...
            if (d.ValueType == "SerialDate" || d.ValueType == "GlobalDayIndex" || d.ValueType == "NewSerialDate") {
                var thisy = 0;
                var footerHeaderFirstText = thisHeaderFooter.append("g")
                    .attr("dctype", uid + '_FirstText')
                    .selectAll('text')
                    .data([0])
                    .join('text')
                    .attr('style', () => {
                        var style = `font-weight:${titleFontBold};font-size:${d.TextFontSize ? d.TextFontSize : defaultData.FontSize}pt;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};`;
                ...
                //[DUWRITER5_0-4743] 20250903 lxy 获取文本实际绘制后的高度，并重新计算垂直居中位置
                footerHeaderFirstText.each(function () {
                    var textNode = d3.select(this);
                    var textBBox = this.getBBox();
                    // 计算垂直居中的新Y坐标
                    var centerY = thisy + (headerFooterLinesSingleHeight[index] / 2) + (textBBox.height / 2) - 1;
                    // 更新文本的Y位置，使其垂直居中
                    textNode.attr('y', centerY);
                });
                ...
                thisHeaderFooter.append("g")
                    ...
                    .data([...new Array(calculationData.NumOfDaysInOnePage).keys()])
                    .join('text')
                    .attr('style', `font-size:${d.TextFontSize ? d.TextFontSize : defaultData.FontSize}pt;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};text-anchor:middle;`)
                    .attr('xml:space', 'preserve')
                    ...
                    .attr('y', () => {
                        ...
                        // thisy = thisy + (calculationData.LableHeight / 2) + (headerFooterLinesSingleHeight[index] / 2);
                        return thisy;
                    });

                //[DUWRITER5_0-4743] 20250903 lxy 获取文本实际绘制后的高度，并重新计算垂直居中位置
                thisHeaderFooter.selectAll('text').each(function () {
                    var textNode = d3.select(this);
                    var textBBox = this.getBBox();
                    // 计算垂直居中的新Y坐标
                    var centerY = thisy + (headerFooterLinesSingleHeight[index] / 2) + (textBBox.height / 2) - 1;
                ...
                thisHeaderFooter.append('g')
                    ...
                    .data(data)
                    .join('text')
                    .attr('style', `font-size:${d.TextFontSize ? d.TextFontSize : defaultData.FontSize}px;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};text-anchor:middle;`)
                    .attr('xml:space', 'preserve')
                    .attr('fill', (a, j) => {
                ...
                if (textList.length > 0) {
                    ...
                    // 计算完text的实际高度后，让text处于居中对齐的位置
                    footerHeaderFirstText.each(function () {
                        var textBBox = this.getBBox();
                        var centerY = thisy + (headerFooterLinesSingleHeight[index] / 2) + (textBBox.height / 2) - 1;
                        d3.select(this).attr('y', centerY);
                    });
                                ...
                                for (var j = 0; j < thisRectValue.text.length; j++) {
                                    ...
                                    if (d.LayoutType == "Cascade") { //Cascade || AutoCascade  水平分割线
                                        ...
                                        dcHeadLineTextG_rect = dcHeadLineTextG.append('rect')
                                            ...
                                            .attr('y', 0);
                                        //刻度线可见
                                        if ((d.TickLineVisible === true || d.TickLineVisible === "true") && j > 0) {
                                            dcHeadLineTextG.append('line')
                                                .attr("x1", j * stepWidth + calculationData.FirstTextWidth + thisRectValue.startIndex)
                                                .attr("y1", 0)
                                                .attr("x2", j * stepWidth + calculationData.FirstTextWidth + thisRectValue.startIndex)
                                                .attr("y2", headerFooterLinesSingleHeight[index])
                                                .attr('stroke', 'black')
                                                .attr('stroke-width', 1);
                                        }
                                    }

                                    if (typeof thisRectValue.text[j] == 'number') {
                                        thisRectValue.text[j] = thisRectValue.text[j] + "";
                                    }
                                    ...
                                    var thisTextEle = dcHeadLineTextG.append('text')
                                        .attr('style', `font-size:${thisRectValue.data[j].TextFontSize ? thisRectValue.data[j].TextFontSize : d.TextFontSize ? d.TextFontSize : defaultData.FontSize}pt;font-family:${thisRectValue.data[j].TextFontName ? thisRectValue.data[j].TextFontName : d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')}`)
                                        .attr('xml:space', 'preserve')
                                        .attr('fill', () => {
                                            var textColor = "black";
                                    ...
                                    //换行子元素的x位置
                                    var needX = dcHeadLineTextG_rectNode.x + 1;//x多加1像素，防止文本压线
                                    //判断溢出文本是自动换行
                                    var FooterLinesCalculateExceedHeight = rootElement.getAttribute("FooterLinesCalculateExceedHeight") || null;
                                    FooterLinesCalculateExceedHeight = FooterLinesCalculateExceedHeight === "true" || FooterLinesCalculateExceedHeight === true;
                                    //当文本宽度大于步长宽度时，是否需要换行展示，并重新计算本行的高度
                                    if (type == "FooterLines" && FooterLinesCalculateExceedHeight) {
                                        //所有文本宽度
                                        var textWidth = thisTextEle.node().getBBox().width;
                                        //单行字体高度
                                        var textHeight = thisTextEle.node().getBBox().height;
                                        //允许绘制的区域宽度 = 步长宽度 - 2个字符的宽(防止文字压线)
                                        // stepWidth = thisStep;//多减2个字符的宽，防止文本溢出或压线
                                        if (textWidth > thisStep) {
                                            //当超出步长宽度时，需要换行展示
                                            var eachRowTextIndex = 0;//每次换行时的位置下标
                                            var eachRowTextArray = [];//重新计算一行可以展示几个字，（特殊字符和数字和中文占据的宽度不一致，需要动态计算）

                                            for (var tIndex = 0; tIndex <= result.length; tIndex++) {
                                                //模拟一个tspan元素，用于获取实际的字体宽度
                                                var thisChar = thisTextEle
                                                    .append('tspan')
                                                    .attr('style', `font-size:${thisRectValue.data[j].TextFontSize ? thisRectValue.data[j].TextFontSize : d.TextFontSize ? d.TextFontSize : defaultData.FontSize}pt;font-family:${thisRectValue.data[j].TextFontName ? thisRectValue.data[j].TextFontName : d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')}`)
                                                    .text(result.substring(eachRowTextIndex, tIndex + 1));
                                                var thisCharWidth = thisChar.node().getBBox().width;
                                                if (thisCharWidth >= thisStep) {
                                                    //当超出步长宽度时，需要换行展示
                                                    eachRowTextArray.push(result.substring(eachRowTextIndex, tIndex));
                                                    eachRowTextIndex = tIndex;
                                                }
                                                thisChar.remove();//删除模拟的tspan元素
                                            }
                                            //剩余字数
                                            if (eachRowTextIndex < result.length) {
                                                eachRowTextArray.push(result.substring(eachRowTextIndex, result.length));
                                            }
                                            //清空已经渲染的文本
                                            thisTextEle.html('');

                                            //当需要换行展示，且没有设置需要的指定高度时，则需要重新计算行高后svg再渲染一次
                                            var alltextLineHeight = Math.ceil(((eachRowTextArray.length * textHeight) * 300 / 96) / 1.3);
                                            if ((!d.SpecifyHeight) || (d.SpecifyHeight < alltextLineHeight)) {
                                                ...
                                                rootElement.SetSpecialItemsProperties(d.UID, {
                                                    SpecifyHeight: alltextLineHeight + 6,//设置指定高度
                                                });
                                                ...
                                                thisTextEle.html(() => {
                                                    var thisTextEleHtml = '';
                                                    for (var rowTex = 0; rowTex < eachRowTextArray.length; rowTex++) {
                                                        //每一行文本
                                                        thisTextEleHtml += `<tspan  x="${needX}" y="${(rowTex + 1) * textHeight}" title="${thisTitle}" style="font-size:${thisRectValue.data[j].TextFontSize ? thisRectValue.data[j].TextFontSize : d.TextFontSize ? d.TextFontSize : defaultData.FontSize}pt;font-family:${thisRectValue.data[j].TextFontName ? thisRectValue.data[j].TextFontName : d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')}">${eachRowTextArray[rowTex]}</tspan>`;
                                                    }
                                        ...
                                        //[DUWRITER5_0-4403] 20250428 lxy  解决文本覆盖的问题
                                        var textInner = thisRectValue.text[j];
                                        //没有设置换行展示时，则直接截取展示（默认展示1行）
                                        for (var k = 0; k < thisRectValue.text[j].length; k++) {
                                            var itemtext = thisRectValue.text[j].substring(0, k + 1);
                                            thisTextEle.text(itemtext);
                                            var thisTextEle_rectNode = thisTextEle.node().getBBox();
                                            if (thisTextEle_rectNode.width > dcHeadLineTextG_rectNode.width) {
                                                textInner = thisRectValue.text[j].substring(0, k);
                                                break;
                                            }
                                        }
                                        ...
                                        if (textInner.indexOf("R") >= 0) {
                                            var reg = new RegExp("R", "g");
                                            textInner = textInner.replace(reg, "®");
                                        }
                                        //修改文本的值
                                        thisTextEle.html(textInner);
                                        thisTextEle.attr("x", () => {
                                            return needX;
                                        });
                                    }
                                    //默认居中展示
                                    var style = `font-family:${thisRectValue.data[j].TextFontName ? thisRectValue.data[j].TextFontName : d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};`;
                                    if (d.LayoutType == "Cascade") {
                                        thisTextEle
                                            .attr('x', (thisRectValue.basisWidth / 2) + calculationData.FirstTextWidth + thisRectValue.startIndex);
                                        style += "text-anchor:middle;";
                                    } else {
                                        var thisTextAlign = (thisRectValue.data[j].TextAlign && thisRectValue.data[j].TextAlign.toLowerCase().trim()) || 'center';
                                        //容器
                                        var thisTextEleRectNodeWidth = dcHeadLineTextG_rectNode.width;
                                        var thisTextEleRectNodeX = dcHeadLineTextG_rectNode.x;
                                        //文本
                                        var thisTextEleWidth = thisTextEle.node().getBBox().width;
                                        if (thisTextAlign == "near") {
                                            thisTextEle
                                                .attr('x', thisTextEleRectNodeX + 1);
                                        } else if (thisTextAlign == 'far') {
                                            thisTextEle
                                                .attr('x', thisTextEleRectNodeX + (thisTextEleRectNodeWidth - thisTextEleWidth) - 1);
                                        } else {
                                            thisTextEle
                                                .attr('x', thisTextEleRectNodeX + ((thisTextEleRectNodeWidth - thisTextEleWidth) / 2));
                                        }
                                    }
                                    thisTextEle.attr('style', () => {

                                        if (result.indexOf("®") >= 0) {
                                            style += `font-size:${thisRectValue.data[j].TextFontSize ? thisRectValue.data[j].TextFontSize : 18}px;`;
                                        } else {
                                            style += `font-size:${thisRectValue.data[j].TextFontSize || d.TextFontSize || defaultData.FontSize}pt`;
                                        }
                                        return style;
                                    });
                                    ...
                                    if (d.LayoutType == "Slant") {
                                        thisTextEle
                                            ...
                                            .attr('style', `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${defaultData.FontSize}px;`);
                                        //计算0 点
                                        ...
                                        dcHeadLineTextG.append('text')
                                            .attr('style', `font-size:${d.TextFontSize ? d.TextFontSize : defaultData.FontSize}px;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};text-anchor:end;`)
                                            .attr('xml:space', 'preserve')
                                            .attr('fill', d.TextColorValue ? d.TextColorValue : "black")
                                        ...
                                        thisTextEle
                                            ...
                                            .attr('style', `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${defaultData.FontSize}px;text-anchor:middle;`);
                ...
                if (d.StartDateKeyword) {
                    ...
                    if (haskeyWordTime) {
                        ...
                        thisHeaderFooter.append('g')
                            ...
                            .join('text')
                            .attr('style', `font-size:${defaultData.FontSize}pt;font-family:${(defaultData.FontName || '宋体,国标宋体')};text-anchor:middle;`)
                            .attr('fill', () => {
            ...
            //增加两端对齐功能justify两端对齐，spacearound两端留白
            var TitleAlign = (d.TitleAlign && d.TitleAlign.toLowerCase().trim()) || '';
            if (['spacearound', 'justify'].indexOf(TitleAlign) >= 0) {
                if (d.Title && d.Title.trim() != "") {
                    //样式
                    var style = `font-weight:${titleFontBold};font-size:${d.TextFontSize ? d.TextFontSize : defaultData.FontSize}pt;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};`;
                    //父元素
                    var dcFirstTextG = footerHeaderFirstText.node().parentElement;
                    //文本内容的宽度
                    var TitleWidth = footerHeaderFirstText.node().getBBox().width;
                    //文本内容的Y
                    var TitleY = footerHeaderFirstText.node().getAttribute("y");
                    //剩余可以填充空白的宽度
                    var JustifyTitleWidth = calculationData.FirstTextWidth - TitleWidth;
                    //每个文本的宽度
                    var eachTextWidth = TitleWidth / d.Title.length;
                    // 开头和结尾处留白的宽度
                    var spacearoundWidth = 0;
                    //当对齐方式为留白两端对齐时，先判断剩余空间够不够两端留白的距离
                    if (TitleAlign === 'spacearound') {
                        if (JustifyTitleWidth >= eachTextWidth) {
                            //足够留白时
                            spacearoundWidth = eachTextWidth;
                        } else if (JustifyTitleWidth > 0) {
                            //不够一个字符的宽度留白，但又有空隙时
                            spacearoundWidth = JustifyTitleWidth;
                        }
                    }

                    //文本间隔中填充的宽度
                    var justifyWidth = (JustifyTitleWidth - spacearoundWidth) / (d.Title.length - 1);
                    justifyWidth = justifyWidth < 0 ? 0 : justifyWidth;

                    //删除原来的文本
                    footerHeaderFirstText.node().remove();

                    //重新添加文本
                    for (var t = 0; t < d.Title.length; t++) {
                        var TitleX = t * justifyWidth + eachTextWidth * t;
                        if (spacearoundWidth > 0) {
                            //开头和结尾处留白
                            TitleX += (spacearoundWidth / 2);
                        }
                        d3.select(dcFirstTextG).append("text")
                            .attr("x", TitleX)
                            .attr("y", TitleY)
                            .attr('style', style)
                            .attr('fill', () => {
    ...
    DrawYAxisInfos: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        calculationData.YAxisInfosData.map((yData, yIndex) => {
            ...
            //当体温单模板为指定模式时，判断Y轴的体温是否需要显示华氏度，如果需要就把宽度放大
            // if (calculationData.TemperatureMode && calculationData.TemperatureMode.toLowerCase() == "fahrenheit") {
            if (thisNameYAxis.ShowInRule == false && calculationData.DCType == "DCTemperatureControlForWASM") {
                return;
            }
            // }
            ...
            var thisYAxisInfos = dcYAxisInfosG.append('g')
                .attr('uid', uid);
            var titleColorValue = yData[1].TitleColorValue || '#000000';
            var fontSize = (yData[1].FontSize && parseFloat(yData[1].FontSize) > 0 ? parseFloat(yData[1].FontSize) : defaultData.FontSize) || 9;
            var dcYAxisInfos_Text = thisYAxisInfos.append('g')
                ...
                .join('text')
                .attr('style', `font-weight:${yData[1].FontBold === 'true' || yData[1].FontBold === true ? 900 : 100};font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${fontSize}pt;fill:${titleColorValue ? titleColorValue : "black"};text-anchor: middle`)
                .html((d, i) => {
                    ...
                    if (i === 0) {
                        ...
                        if (yData[1].ShowLegendInRule === true || yData[1].ShowLegendInRule === 'true') {
                            ...
                            // symbolSize = parseFloat((symbolSize / 300 * 96.00001209449).toFixed(2));
                        ...
                        if (yData[1].Title) {
                            ...
                            thisYAxisInfos.append('g')
                                ...
                                .data([0])
                                .join('text')
                                .attr('style', `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${fontSize}pt;fill:${titleColorValue ? titleColorValue : "black"};text-anchor: middle`)
                                .attr("x", () => {
                    ...
                    //隐藏最小值
                    if (yData[1].MinValueVisible == 'false' || yData[1].MinValueVisible == false) {
                        if (d === yData[1].MinValue) {
                            d = '';
                        }
                    }
                    //隐藏最大值
                    if (yData[1].MaxValueVisible == 'false' || yData[1].MaxValueVisible == false) {
                        if (d === yData[1].MaxValue) {
                            d = '';
                        }
                    }
                ...
                .attr('y', (d, i) => {
                    ...
                    var rectHeight = calculationData.YAxisInfosTotalHeight - thisNameYAxis.YTopHeight - thisNameYAxis.YBottomHeight;
                    var splitNum = thisNameYAxis.YSplitNum || 1;
                    var StepHeight = rectHeight / splitNum;
                    return thisNameYAxis.YTopHeight + (StepHeight * i);
            ...
            AllText.forEach((item) => {
                var x = item.getAttribute("x") && parseFloat(item.getAttribute("x")) || 0;
                var y = item.getAttribute("y") && parseFloat(item.getAttribute("y")) || 0;
                var itemWidth = item.getBBox().width; //获取当前元素的宽度
                var itemHeight = item.getBBox().height; //获取当前元素的宽度
                //没有指定位置时，垂直位置矫正

                if (!thisNameYAxis.Scales || !thisNameYAxis.Scales.length) {
                    var dataIndex = item.getAttribute("data-index");
                    if (dataIndex !== null) {
                        dataIndex = parseInt(dataIndex);
                        //是否需要纠正最大值和最小值的位置，默认纠正（为了不让刻度超出边界）
                        var CorrectTheVerticalOfTickTextMaxMin = yData[1].CorrectTheVerticalOfTickTextMaxMin && yData[1].CorrectTheVerticalOfTickTextMaxMin === true;
                        if (dataIndex === 0) {
                            if (CorrectTheVerticalOfTickTextMaxMin === false) {
                                y = y + (itemHeight / 2) - 2;//居中
                            } else {
                                y = y + itemHeight - 2;//不超出顶部
                            }

                        } else if (dataIndex === (yData[0].length - 1)) {
                            if (CorrectTheVerticalOfTickTextMaxMin === false) {
                                y = y + (itemHeight / 2) - 2;//居中
                            } else {
                                y -= 2;//不超出底部
                        ...
                        item.setAttribute("y", y);//设置y坐标
                    }
                }

                //开启了摄氏度体温单
                let IsFahrenheit = calculationData.TemperatureMode && calculationData.TemperatureMode === "Fahrenheit";
                //当前Y轴展示了摄氏度体温单
                let IsFahrenheitVisible = thisNameYAxis.FahrenheitVisible && thisNameYAxis.FahrenheitVisible === true;
                if (IsFahrenheit && IsFahrenheitVisible) {
                    //如果是刻度体温单并且当前的Y轴开启了展示标尺，则需要将文本靠右展示
                    x = x + ((calculationData.YAxisInfosSingleWidth[index] / 2));
                    item.setAttribute("x", x * 1.5); //靠右边的位置
                } else {

                    var RightPadding = yData[1].RightPadding && yData[1].RightPadding > 0 ? yData[1].RightPadding : 0;
                    var LeftPadding = yData[1].LeftPadding && yData[1].LeftPadding > 0 ? yData[1].LeftPadding : 0;
                    if (TickTextAlignment === 'Near') {
                        //左对齐
                        x = (1 + x + (itemWidth / 2));
                        x += LeftPadding;//间距
                        ...
                        //右对齐
                        x = x + (calculationData.YAxisInfosSingleWidth[index] - (itemWidth / 2)) - 2;
                        x -= RightPadding;//间距
                        item.setAttribute("x", x);
                    } else {
                        //居中对齐
                        x = x + ((calculationData.YAxisInfosSingleWidth[index] / 2));
                        item.setAttribute("x", x - 2);
                    }
                }
            });
            ...
            if (yData[1].MergeIntoLeft) {
                if (yData[1].TopPadding > 0 && yData[1].BottomPadding < 1) {
                    ...
                    thisYAxisInfos.append('g')
                        ...
                        .attr('stroke', ISDefaultTemperatureMode ? "black" : "none");
                }
            } else {
            ...
            //20250324 新增，绘制华氏度刻度尺
            if (thisNameYAxis.FahrenheitVisible && thisNameYAxis.FahrenheitVisible === true) {
                //最大值是否展示
                var maxValueVisible = thisNameYAxis.MaxValueVisible && thisNameYAxis.MaxValueVisible === true;
                //可绘制区域的高度
                var rectHeight = calculationData.YAxisInfosTotalHeight - thisNameYAxis.YTopHeight - thisNameYAxis.YBottomHeight;
                //可绘制区域的宽度
                var rectWidth = calculationData.YAxisInfosSingleWidth[index];
                //华氏度绘制的的开始位置 默认从顶部开始绘制，如果隐藏了最大值，就从第二大值的位置开始绘制
                var rectY = thisNameYAxis.YTopHeight;

                //最大值最小值
                let maxC = parseFloat(thisNameYAxis.MaxValue);
                let minC = parseFloat(thisNameYAxis.MinValue);
                //计算每个数值之间的间隔大小
                var YSplitNum = thisNameYAxis.YSplitNum || 1;
                var stepNum = (maxC - minC) / YSplitNum;
                //计算每个数值之间的间隔高度
                var stepHeight = rectHeight / YSplitNum;

                //隐藏了最大值,刻度也不需要绘制
                if (maxValueVisible === false) {
                    //摄氏度绘制区域：剪掉最大值绘制区域的高度
                    rectHeight -= stepHeight;
                    //摄氏度开始绘制的Y坐标
                    rectY += stepHeight;
                    //最大值使用第二个数
                    maxC = YSplitNum > 1 ? maxC - stepNum : maxC;
                }

                //华氏度 = 32+ 摄氏度 × 1.8
                var minF = parseFloat((32 + minC * 1.8).toFixed(1));
                var maxF = parseFloat((32 + maxC * 1.8).toFixed(1));
                //每一个刻度的高度
                var StepFHeight = (rectHeight / (maxF - minF));

                //追加一个可以填满整个父元素的块
                thisYAxisInfos.append('rect')
                    .attr('x', 0)
                    .attr('y', rectY)
                    ...
                    .attr('stroke', 'none')
                    .attr('stroke-width', 1);
                let dcYAxiosInfosFahrenheitG = thisYAxisInfos.append('g')
                    .attr("dctype", "dcYAxiosInfos_Fahrenheit")
                    .attr('transform', `translate(0,${rectY})`);



                //最底部标尺线段不超出
                var minFStartY = (Math.ceil(minF) - minF) * StepFHeight;
                dcYAxiosInfosFahrenheitG.append('line')
                    .attr('x1', rectWidth / 2)
                    .attr('y1', 0)
                    .attr('x2', rectWidth / 2)
                    .attr('y2', rectHeight - minFStartY)
                    .attr('stroke', "black")
                    .attr('stroke-width', 1);



                for (var f = Math.ceil(minF); f <= maxF; f += 1) {
                    var FtextElement = dcYAxiosInfosFahrenheitG.append('text')
                        .attr('x', (rectWidth / 2) - 24)
                        .attr('y', rectHeight - ((f - minF) * StepFHeight) - 2)
                        .attr('fill', "black")
                        .attr('style', `font-weight:${yData[1].FontBold === 'true' || yData[1].FontBold === true ? 900 : 100};font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${fontSize}pt;fill:${titleColorValue ? titleColorValue : "black"};text-anchor: middle`)
                        .text(f);
                    var FtextElementHeight = FtextElement.node().getBBox().height;
                    var FtextElementY = FtextElement.node().getAttribute('y');
                    FtextElementY = parseFloat(FtextElementY) + (FtextElementHeight / 2);
                    FtextElement.attr('y', FtextElementY); //垂直居中

                    //绘制刻度尺
                    dcYAxiosInfosFahrenheitG.append('line')
                        .attr('x1', rectWidth / 2 - 10)
                        .attr('y1', rectHeight - ((f - minF) * StepFHeight))
                        .attr('x2', rectWidth / 2)
                        .attr('y2', rectHeight - ((f - minF) * StepFHeight))
                        .attr('stroke', "black")
                        .attr('stroke-width', 1);
                    ...
                    if (f < maxF - 1) {
                        ...
                        for (var l = 0; l < 5; l++) {
                            if (l !== 0) {
                                var stepMiniHeight = StepFHeight / 5;
                                //绘制刻度尺
                                dcYAxiosInfosFahrenheitG.append('line')
                                    .attr('x1', rectWidth / 2 - 6)
                                    .attr('y1', rectHeight - ((f - minF) * StepFHeight) - (stepMiniHeight * l))
                                    .attr('x2', rectWidth / 2)
                                    .attr('y2', rectHeight - ((f - minF) * StepFHeight) - (stepMiniHeight * l))
                                    .attr('stroke', defaultData.GridLineColorValue || 'black')
                                    .attr('stroke-width', 1);
                            }
            ...
            for (var j = 0; j < value.length; j++) {
                ...
                if (minMax) {
                    ...
                    if (value[j].Datas) {
                        ...
                        //空心目标转换
                        if (minMax.HollowCovertTargetName) {
                            var hollowCovertTargetName = minMax.HollowCovertTargetName.split(',');
                            if (hollowCovertTargetName.length >= 1) {
                                for (var l = 0; l < hollowCovertTargetName.length; l++) {
                                    var hollowCovertTarget = hollowCovertTargetName[l];
                                    var targetNameDOM = dcValuePathG.node().querySelector(`[dctype=dcValuePath_${hollowCovertTarget}]`) || null;
                        ...
                        thisSvgDataArr.forEach((thisSvgData) => {
                            ...
                            if (minMax.Style == "Text") {
                                thisValuePath.append("g")
                                    ...
                                    .attr('style', (d, i) => {
                                        ...
                                        return `font-size:${thisSvgData[i].TextFontSize || defaultData.FontSize || 9}pt;font-family:${thisSvgData[i].TextFontName || (defaultData.FontName || '宋体,国标宋体')}; writing-mode: vertical-lr; /* 从上到下书写，从左到右 */
                                    ...
                                    .html((d, i) => {
                                        ...
                                        if (newResultHtml && newResultHtml.length) {
                                            ...
                                            //文本支持自定义Title
                                            if (thisSvgData[i].Title) {
                                                var titletext = thisSvgData[i].Title;
                                            } else {
                                                var titletext = thisSvgData[i].Time + " " + thisSvgData[i].Text;
                                            }
                                            titletext = titletext.replace(/&dc&/g, ',');
                                            //[DUWRITER5_0-4175] 20250212 lxy 自定义画点Title是否展示
                                            var ShowTip = thisSvgData[i].ShowTip;
                                            ...
                                            //如果是线性模式，并且没有设置ExcludeTextAndEnableDataGridLinearAxisMode，则不需要加上格子宽度
                                            if (defaultData.EnableDataGridLinearAxisMode == true && !defaultData.ExcludeTextAndEnableDataGridLinearAxisMode) {
                                                // xValue保持不变
                                            } else {
                                                //默认让文本处于格子的中间位置    
                                                var textFontSize = parseFloat(defaultData.FontSize) || 9;
                                                xValue += (textFontSize / 2);
                                                xValue += ((calculationData.VerticalStepWidth - textFontSize) / 2);
                                            }

                                            var charArr = '';
                                            for (var k = 0; k < newResultHtml.length; k++) {
                                                var char = newResultHtml[k];
                                                //文字之间的间隔
                                                var margin = (calculationData.HorizontStepHeight - calculationData.VerticalStepWidth) || 0;
                                                if (margin > 0) {
                                                    margin = margin / 2;
                                                } else {
                                                    margin = 0;
                                                ...
                                                if (thisSvgData[i].TextFontSize) {
                                                    //如果指定了字体大小，就优先用指定字体大小
                                                    var minSize = parseFloat(thisSvgData[i].TextFontSize);
                                                } else {
                                                    //没有指定格子大小，则取取格子宽高的最小值，作为字体大小,用来保证文字在格子内
                                                    var minSize = Math.min(calculationData.VerticalStepWidth, calculationData.HorizontStepHeight);
                                                }
                                                var style = char.match(/[a-zA-Z0-9]/g) ? `font-size:${minSize}pt;` : `font-size:${minSize}px;writing-mode: lr;`;

                                                var thisY = y;
                                                var num = Math.round(y / minSize);//四舍五入取整数，保证文字在格子内,防止压线
                                                thisY = num * minSize;
                                                //判断是否需要间距
                                                if (calculationData.HorizontStepHeight > minSize) {
                                                    thisY += (calculationData.HorizontStepHeight - minSize) / 2;
                                                }
                                                thisY += (minSize * k);
                                                charArr += `<tspan x="${xValue}" y="${thisY}"  ${ShowTip === false ? `` : `title="${titletext}"`}   style="${style}" >${char}</tspan>`;
                                            }



                                            //将所有文字用tspan包裹，正向渲染，避免旋转
                                            // newResultHtml = newResultHtml.split('').map(char => `<tspan x="10" y="20" ${ShowTip === false ? `` : `title="${titletext}"`}   style="${char.match(/[a-zA-Z0-9]/g) ? '' : 'writing-mode: lr;'}" >${char}</tspan>`).join('');
                                        }
                                        return charArr;
                                    })
                                    ...
                                    .attr('x', (d, i) => {
                                        ...
                                        //修改了一次文本赋值，x在整点时位置有偏移的问题 20250811 lxy [DUWRITER5_0-4712]
                                        var thisTime = new Date(thisSvgData[i].Time).getTime();
                                        if (thisBand.indexOf(thisTime) < 0) {
                                            thisBand.find((x, y) => {
                                                if (x >= thisTime) {
                                ...
                                //对text 根据y轴的大小进行排序
                                var allGroupText = thisValuePath.node().querySelectorAll("text");
                                if (allGroupText && allGroupText.length > 0) {
                                    //循环y轴相同时在某个文本区间内向下移动
                                    allGroupText = Array.prototype.slice.call(allGroupText);
                                    allGroupText = allGroupText.sort((a, b) => {
                                        return parseFloat(a.getAttribute('y')) - parseFloat(b.getAttribute('y'));
                                    });
                                    //双循环判断
                                    for (var z = 1; z < allGroupText.length; z++) {
                                        for (var y = 0; y < z; y++) {
                                            var compareText = allGroupText[y];
                                            var targetText = allGroupText[z];
                                            //在x轴相同的情况下比较y轴
                                            if (compareText.getAttribute('x') == targetText.getAttribute('x')) {
                                                var newy = parseFloat(targetText.getAttribute('y'));
                                                var oldy = newy;
                                                var comparey = parseFloat(compareText.getAttribute('y'));
                                                var compareHeight = parseFloat(compareText.getBBox().height);
                                                if (newy >= comparey && newy < comparey + compareHeight) {
                                                    //判断是否存在
                                                    var hasGapLine = targetText.querySelector(".dc_textGapLine");
                                                    if (hasGapLine == null) {
                                                        newy = comparey + compareHeight + 5;
                                                    } else {
                                                        newy = comparey + compareHeight - 12 + 5;
                                                    }
                                                    //console.log(calculationData.YAxisInfosTotalHeight)
                                                    //如果超出直接隐藏
                                                    if (newy >= calculationData.YAxisInfosTotalHeight) {
                                                        //targetText.setAttribute("fill", 'transparent')
                                                        targetText.style.display = "none";
                                                    } else {
                                                        //修改内部tspan高度
                                                        var alltspan = targetText.querySelectorAll('tspan');
                                                        targetText.setAttribute("y", newy);
                                                        alltspan = Array.prototype.slice.call(alltspan);
                                                        alltspan.forEach((tspan) => {
                                                            var textY = parseFloat(tspan.getAttribute('y')) - oldy + newy;
                                                            tspan.setAttribute("y", textY);
                                                            textY = textY + tspan.getBBox().height;
                                                            //判断是否为超出边界

                                                            if (textY > calculationData.YAxisInfosTotalHeight) {
                                                        ...
                                                        if (hasGapLine == null) {
                                                            targetText.insertAdjacentHTML("afterbegin", `<tspan class="dc_textGapLine" style="font-size: 12px" y="${newy - 7}" fill="#000">一</tspan`);
                                                        } else {
                                                            hasGapLine.setAttribute("y", newy - 7);
                                                        }
                                                    }
                                                }
                                            }
                            ...
                            const line = d3
                                ...
                                .y((i) => {
                                    ...
                                    allPointIndex.forEach((item) => {
                                        //截取两位小数
                                        thisXValue = parseFloat(thisXValue).toFixed(2);
                                        item.x = parseFloat(item.x).toFixed(2);
                                        yValue = parseFloat(yValue).toFixed(2);
                                        item.y = parseFloat(item.y).toFixed(2);
                                        //对第二位小数四舍五入
                                        thisXValue = Math.round(thisXValue * 10) / 10;
                            ...
                            for (var z = 0; z < thisSvgData.length; z++) {
                                ...
                                var IconData = {
                                    ...
                                    y: (i) => {
                                        ...
                                        if (minMax.ShowPointValue === true || thisSvgData[i].ShowPointValue === true) {
                                            ...
                                            thisValuePath.append("text")
                                                .attr('style', `font-family:宋体,国标宋体;font-size:9px;text-anchor:middle;`)
                                                .attr('fill', 'black')
                                        ...
                                        if (IconData.sign && thisSvgData[i].LanternValue && thisSvgData[i].LanternValue >= 0 && minMax.EnableLanternValue !== false) {
                                            ...
                                            var lanternY = yScale(thislanterValue);//灯笼值高度
                                            //画灯笼线
                                            var lanternLine = null;
                                            if (thisSvgData[i].LineStyleForLanternValue !== "None") {
                                                lanternLine = thisValuePath.append('line')
                                                    //.attr('fill', 'stroke')
                                                    .attr('x1', thisX)
                                                    .attr('y1', thisY)
                                            ...
                                            var LanternValueText = thisValuePath.append('text')
                                                .attr('style', `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-weight:${thisSvgData[i].TextFontWeight ? thisSvgData[i].TextFontWeight : 'normal'};font-size:${thisSvgData[i].CharLantern == "R" ? 14 : (thisSvgData[i] && thisSvgData[i].TextFontSize ? thisSvgData[i].TextFontSize : defaultData.FontSize)}pt;text-anchor:middle;`)
                                                .attr('fill', () => {
                                                ...
                                                .attr("title", () => {
                                                    //[DUWRITER5_0-4131] 20250212 lxy 光标移入灯笼值展示灯笼值内容
                                                    if (thisSvgData[i].LanternTitleText && thisSvgData[i].LanternTitleText.length) {
                                                        return thisSvgData[i].LanternTitleText;
                                                    }
                                            ...
                                            // [DUWRITER5_0-4159] 20250212 lxy 修复灯笼字符位置垂直居中
                                            var lanternTextBBox = LanternValueText.nodes()[0].getBBox();
                                            if (lanternTextBBox.height) {
                                                // 常量，来表示Y坐标的调整值
                                                const Y_ADJUSTMENT = 1;
                                                var lanternTextY = lanternY + (lanternTextBBox.height / 2) - Y_ADJUSTMENT;

                                                // 重新设置文本的Y坐标
                                                LanternValueText.attr('y', lanternTextY);

                                                // 重新设置灯笼线段的Y2位置
                                                lanternLine && lanternLine.attr('y2', lanternY);
                                            }

                                        }

                                        //复测标记
                                        if (IconData.sign && thisSvgData[i].EnableRetestMarkValue === true) {

                                            //计算标记大小
                                            var CharRetestMarkFontSize = thisSvgData[i].CharRetestMarkFontSize;
                                            CharRetestMarkFontSize = CharRetestMarkFontSize ? parseFloat(CharRetestMarkFontSize) : 20;
                                            // CharRetestMarkFontSize = parseFloat((CharRetestMarkFontSize / 300 * 96.00001209449).toFixed(2));


                                            //是否设置了复测标记的值，如果没有则默认放在value值的上方
                                            if (!thisSvgData[i].RetestMarkValue) {
                                                var thisResetMaskValue = Number(thisSvgData[i].Value) + 0.0001;
                                            } else {
                                                var thisResetMaskValue = Number(thisSvgData[i].RetestMarkValue);
                                            }
                                            if (thisResetMaskValue >= parseFloat(minMax.MaxValue)) {
                                                thisResetMaskValue = parseFloat(minMax.MaxValue) - 0.0001;
                                            } else if (thisResetMaskValue <= parseFloat(minMax.MinValue)) {
                                                thisResetMaskValue = parseFloat(minMax.MinValue) + 0.0001;
                                            }
                                            var resetMaskY = yScale(thisResetMaskValue);

                                            thisValuePath.append('text')
                                                .attr('style', () => {
                                                    var fontSize = (thisSvgData[i] && thisSvgData[i].TextFontSize ? thisSvgData[i].TextFontSize : defaultData.FontSize);
                                                    return `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-weight:${thisSvgData[i].TextFontWeight ? thisSvgData[i].TextFontWeight : 'normal'};font-size:${CharRetestMarkFontSize ? CharRetestMarkFontSize : fontSize}pt;text-anchor:middle;`;

                                                })
                                                .attr('fill', () => {
                                                    if (thisSvgData[i].CharRetestMarkColor) {
                                                        return thisSvgData[i].CharRetestMarkColor;
                                                    }
                                                    //物理升温
                                                    if ((parseFloat(thisSvgData[i].RetestMarkValue) > parseFloat(thisSvgData[i].Value))) {
                                                        if (minMax.LanternValueColorForUpValue) {
                                                            return minMax.LanternValueColorForUpValue;
                                                        } else {
                                                            return "rgba(0,0,255,0.6)";
                                                ...
                                                .attr('x', () => {
                                                    if (thisSvgData[i].CharRetestMarkAlign == "left") {
                                                        return thisX - symbolSize;
                                                    } else if (thisSvgData[i].CharRetestMarkAlign == "right") {
                                                        return thisX + symbolSize;
                                                    }
                                                    return thisX;
                                                })
                                                .attr('y', () => {
                                                    return (resetMaskY - Number(defaultData.FontSize) / 2) - 2;
                                                })
                                                .text(() => {

                                                    return thisSvgData[i].CharRetestMark || "V";
                                                })
                                                .attr("title", () => {
                                    ...
                                    title: ((i) => {
                                        //[DUWRITER5_0-4175] 20250212 lxy 自定义画点Title是否展示
                                        if (thisSvgData[i].ShowTip === false) {
                                            return null;
                                        }
                                        //[DUWRITER5_0-3404]20240826 lxy 自定义画点Title
                                        var titleText = thisSvgData[i].Time + " " + minMax.Title + " " + thisSvgData[i].Value + `${thisSvgData[i].LanternValue > 0 ? ((parseFloat(thisSvgData[i].LanternValue) > parseFloat(thisSvgData[i].Value)) ? ' ⬆ ' : ' ⬇ ') + thisSvgData[i].LanternValue : ''}`;
                ...
                if (nameG && valueG) {
                    ...
                    if (nameG && valueG) {
                        ...
                        if (nameCoordinate.length > 0 && valueCoordinate.length > 0) {
                            ...
                            //[DUWRITER5_0-4217] 20250304 lxy 修复阴影区域绘制错误
                            if (CoordinateArr.length > 0) {
                                ...
                                if (minMax && minMax.ShadowStyle && (minMax.ShadowPointVisible === true || minMax.ShadowPointVisible === 'true')) {
        ...
        return {
            ...
            P: (({
                ...
                content
                    ...
                    .join('text')
                    .attr('x', x)
                    .attr('y', y) // 假设 y 是你希望的垂直居中的位置
                    .attr('font-size', r * 4) // 调整字体大小以适应 r 的值
                    .attr('fill', fill)
                    .attr('stroke', stroke)
                    .attr('title', (i) => {
                        if (typeof title === 'function') {
                    ...
                    .attr('dcSymbolX', x)
                    .attr('dcSymbolY', y)
                    .attr('dy', +(r * 2)) // 调整 dy 以垂直居中，这里假设文本高度大约为 r * 4，所以减去高度的一半 r * 2
                    .text('P'); // 直接使用字符 'P'
    ...
    MoveBorderRect: function (rootElement, uid) {
        ...
        if (borderSpan == null) {
            ...
            borderSpan.style.boxSizing = "border-box";
            borderSpan.style.padding = "0";
            borderSpan.style.margin = "0";
    ...
    //绘制占位的快元素
    DrawBlockElements: function (svg, calculationData, defaultData, svgEle, i) {
        //记录体温单模式 Fahrenheit:带有刻度和右侧占位且隐藏左侧边框的体温单，default：默认模式的体温单
        if (calculationData.TemperatureMode && calculationData.TemperatureMode === "Fahrenheit") {
            var blockHeight = calculationData.TableContainerHeight;//绘制区域的高
            var blockWidth = defaultData.SpecifyRightPlaceholderElementWidth || 16;
            blockWidth = Math.round(parseFloat(blockWidth) / 25.4 * 96);
            var blockX = calculationData.LeftMargin + calculationData.InnerWidth;
            var blockY = calculationData.TopMargin + calculationData.SpecifyTitleHeight + (calculationData.LableHeight * calculationData.AllHeaderLabels.length);
            var blockG = svg.append('g')
                .attr('dctype', "dcBlockG")
            ...
            //页脚处画一条线
            var footerLineY = blockHeight - calculationData.FooterLinesTotalHeight;
            var footerLine = blockG.append('line')
                .attr('x1', 0)
                .attr('y1', footerLineY)
                .attr('x2', blockWidth)
                .attr('y2', footerLineY)
        ...
        for (var i = 1; i <= maxRow; i++) {
            ...
            newJson.map(item => {
                if (item.rowLevel === i) {
                    ...
                    th.style.fontFamily = item.fontname ? item.fontname : '宋体,国标宋体';

...
export let TemperatureControl_Designer = {
    ...
    initDesignerDiv: function (rootElement, xml) {
        ...
        if (CreateTemperatureControlForWASM) {
            ...
            if (TitleLineSerialDateCompletion === 'true' || TitleLineSerialDateCompletion === true) {
                DesignerRootElement.setAttribute("TitleLineSerialDateCompletion", "true");
            }
            ...
            //体温单当特殊项目值超过高度时，是否允许自动撑开高度换行展示
            var FooterLinesCalculateExceedHeight = rootElement.getAttribute("FooterLinesCalculateExceedHeight") || null;
            if (FooterLinesCalculateExceedHeight) {
                DesignerRootElement.setAttribute("FooterLinesCalculateExceedHeight", FooterLinesCalculateExceedHeight);
            }
            ...
            DesignerTree.addEventListener('click', function (e) {
                ...
                if (e.target.className.indexOf('dc_designer_tree_item_title_text') >= 0) {
                    ...
                    if (attrId) {
                        ...
                        //[DUWRITER5_0-4272] 20250321 增加点击目录树，体温单选中元素滚动到可视区域
                        var currentSelectElement = DesignerRootElement.querySelector(`svg [uid="${attrId}"]`);
                        if (currentSelectElement) {
                            currentSelectElement.scrollIntoView({ behavior: "smooth", block: "center", inline: "center" });
                        }
                    }
            ...
            dcDesignerExportButton && dcDesignerExportButton.addEventListener('click', function () {
                let DesignerRootElement = that.GetDesignerRootElement();
                DesignerRootElement.SaveTemperatureDocumentToFile(null, 'json');
            });
    ...
    */
    ConfirmDesigner: function () {
        ...
        if (isUnsave) {
            if (confirm('当前设计器有未保存的修改，是否需要应用后再确认？')) {
                console.log('应用之后再确认');
        ...
        //文档字符串
        var jsonString = rootElementDesigner.SaveTemperatureDocumentToString('json');
        if (rootElement) {
            ...
            if (rootElement && rootElement.EventDesignerControlConfirm && typeof rootElement.EventDesignerControlConfirm === 'function') {
                rootElement.EventDesignerControlConfirm(documentConfig, rootElementDesigner.DocumentOptions.DefaultData.Config, jsonString);
            } else {
                //让体温单加载修改过的文档
                rootElement.LoadTemperatureDocumentFromString(jsonString);
                //删除设计器元素
    ...
    */
    CreatDesignerPropertyDom: function (attrId = null) {
                ...
                if (DCDesignerDocumentObject[attrType] && DCDesignerDocumentObject[attrType][key] && DCDesignerDocumentObject[attrType][key].type) {
                    ...
                    switch (inputType) {
                        ...
                        case 'number':
                            ...
                            formInput = `<input attr-option-name="${key}" class="dc_designer_options_item_value_input"  type="number" value="${(propValue || propValue === 0) ? propValue : ''}"></input>`;;
                            break;
                        case 'select':
        ...
        localStorage.setItem('dc_designer_form_prop', JSON.stringify(formCurrentDesignerElementProp));
    },
    /**
   * 获取属性对象，并渲染到右侧属性面板
   ...
   */
    ...
    */
    ...
    GetDesignerFormPropUnsave: function () {
        //获取右侧的表单数据
        var formCurrentDesignerElementProp = this.GetDesignerFormProp();
        //当前节点初始化表单
        var dc_designer_form_prop = localStorage.getItem('dc_designer_form_prop');
        if (dc_designer_form_prop != JSON.stringify(formCurrentDesignerElementProp)) {
            console.log('有修改未保存');
            return true;
        }
        return false;
    ...
    */
    DCDesignerDocumentObjectCN: {
        ...
        "AfterOperaDaysFromZero": "从0开始的操作天数",
        "AfterOperaDaysBeginOne": "从1开始的操作天数",
        ...
        "MinValueVisible": "在Y轴中显示最小值",
        "MaxValueVisible": "在Y轴中显示最大值",
        "FahrenheitVisible": "在Y轴中显示摄氏度标尺",
        "CorrectTheVerticalOfTickTextMaxMin": "在Y轴中最大值和最小值不超出刻度范围",
        "ShowInRule": "在标尺中显示当前Y轴",
        "RightPadding": "Y轴刻度右内边距",
        "LeftPadding": "Y轴刻度左内边距",
        "FontBold": "刻度字体加粗",
        "SpecifyRightPlaceholderElementWidth": "右侧占位区域的宽度",
        "TemperatureMode": "体温单模式",
        "RightPlaceholderElement": "展示右侧占位区域",
        "TitleFontBold": "标题加粗",
    }
};
// 记录特殊的设计器属性
var DCDesignerDocumentObject = {
    document: {
        ...
        TemperatureMode: {
            type: "select",
            options: ["Defalut", "Fahrenheit"]
        },
        SpecifyRightPlaceholderElementWidth: {
            type: "number",
        },
        RightPlaceholderElement: {
            type: "boolean",
        }
    },
    YAxisInfos: {
        ...
        SymbolStyle: {
            type: 'select',
            options: ['SolidCicle', 'HollowCicle', 'Cross', 'SolidTriangle', 'SolidTriangleReversed', "HollowSquare"]
            // : ["SolidCicle",
            //     "Cross",
            //     // "RoundDotIcon",
            //     "SolidTriangle",
        ...
        CorrectTheVerticalOfTickTextMaxMin: {
            type: "boolean"
        },
        ShowInRule: {
            type: "boolean"

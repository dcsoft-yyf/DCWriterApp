 ...
 * @UserDefinedOptions ["CopyNoKeepReadOnlyImages", "true/false", "复制时是否不保留只读的图片"]
 * @UserDefinedOptions ["CustomFontFamily", "", "参数为数组用于表示网路字体名称和路径"]
...
export let WriterControl_API = {
    ...
    */
    BindControlForCommon: function (rootElement, refDCWriter) {
       ...
       */
        ...
        */
        ...
        /**
         * @name getVisiblePageNumbers
         * @type function
         * @apinameZh 获取当前可视区域内的页面页码号数组，页码从0开始
         * @classification view
         * @returns ["result","array","可视页面页码号数组"]
         */
        rootElement.getVisiblePageNumbers = function () {
            // 初始化一个数组，用于存储可视区域内的页面页码号
            ...
            /** 正文元素 */
            var pageContainer = rootElement.querySelector(SelectStr);
            if (!pageContainer) {
            ...
            if (!allPagesNode || allPagesNode.length === 0) {
                return VisiblePageNumbers;
            }
            // 获取编辑或者打印预览盒子的DOMRect对象
            var DivRect = pageContainer.getBoundingClientRect();
            for (var i = 0; i < allPagesNode.length; i++) {
                var PageNode = allPagesNode[i];
                // 获取元素的DOMRect对象
                ...
                // 将页面页码号添加到可视页面页码号数组中
                VisiblePageNumbers.push(PageNode.PageIndex);
            }
            // 返回可视页面页码号数组
            return VisiblePageNumbers;
        };
    ...
    BindControlForWriterControlForWASM: function (rootElement, refDCWriter) {
         ...
         * @type function
         * @appnameZh 获取指定表格所有修改过的表格行对象集合
         * @param ["tableelement","object","指定表格元素的id或nativehandle或后台引用对象","","",null]
         * @classification view
         * @change ["2025-4-1","新增接口DUWRITER5_0-4310","wyc" ]
         */
        rootElement.getModifiedTableRows = function (tableelement) {
            var startTime = new Date();
            var result = null;
            if (DCTools20221228.IsDotnetReferenceElement(tableelement) === true) {
                result = rootElement.__DCWriterReference.invokeMethod("GetModifiedTableRows2", tableelement);
            } else {
         ...
         * @name LoadSave
         * @type function
         * @appnameZh 将当前文档保存后再直接加载的方便函数
         * @classification view
         * @change ["2025-3-31","新增接口","wyc" ]
         */
        rootElement.LoadSave = function () {
            var startTime = new Date();
            rootElement.LoadDocumentFromString(rootElement.SaveDocumentToString(), "xml");
            DCEventInterfaceLogFunction(rootElement, 'LoadSave', startTime);
            return;
        };

        /**
         * @name GetFirstChildElementByType
         * @type function
         * @appnameZh 用指定样式的删除线标记表格行元素
         * @classification view
         * @param ["element","object","指定容器元素的id或nativehandle或后台引用对象","","",null]
         * @param ["typename","string","指定元素的类型名称","","",true]
         * @param ["isdispose","boolean","后台是否释放资源默认释放","","",true]
         * @change ["2025-3-26","新增接口","wyc" ]
         */
        rootElement.GetFirstChildElementByTypeName = function (element, typename, isdispose = true) {
            var startTime = new Date();
            var result = null;
            if (DCTools20221228.IsDotnetReferenceElement(element) === true) {
                result = rootElement.__DCWriterReference.invokeMethod("GetFirstChildElementBYType2", element, typename, isdispose);
            } else {
                result = rootElement.__DCWriterReference.invokeMethod("GetFirstChildElementBYType", element, typename);
            }
            DCEventInterfaceLogFunction(rootElement, 'GetFirstChildElementByType', startTime);
            return result;
        };

        /**
         * @name MarkTableRowDeletorLine
         * @type function
         * @appnameZh 用指定样式的删除线标记表格行元素
         * @classification view
         * @param ["element","object","指定表格行元素的id或nativehandle或后台引用对象","","",null]
         * @param ["options","object","标记删除线的样式若传null则为取消标记","","",true]
         * @change ["2025-3-25","新增接口","wyc" ]
         */
        rootElement.MarkTableRowDeletorLine = function (element, options) {
            var startTime = new Date();
            var result = false;
            if (element == null) {
                return false;
            ...
            if (options === null || options === undefined) {
                rootElement.RemoveElementAttributes(element, "$DCADVISE$", false);
                rootElement.EditorRefreshContainerView(element);
                return true;
            }
            var strs = null;
            if (typeof (options) === "string") {
                strs = options.toString();
            }
            else if (typeof (options) === "object") {
                strs = options.DashStyle.toString() + " " + options.Color.toString() + " " + options.Width.toString();
            }
            if (strs == null) {
                return false;
            }
            result = rootElement.SetElementAttribute(element, "$DCADVISE$", strs, false);
            rootElement.EditorRefreshContainerView(element);
            DCEventInterfaceLogFunction(rootElement, 'MarkTableRowDeletorLine', startTime);
            return result;
        };


        /**
         * @name SetElementLogicDeleted
         * @type function
         * @appnameZh 设置文档元素被逻辑删除
         * @classification view
         * @param ["element","object","指定元素的id或nativehandle或后台引用对象","","",null]
         * @param ["logUndo","boolean","是否记录撤销操作","","",true]
         * @param ["updateView","boolean","是否更新视图","","",true]
         * @change ["2025-3-12","新增接口","wyc" ]
         * @returns ["result","boolean","代表函数是否被成功执行"]
         */
        rootElement.SetElementLogicDeleted = function (element, logUndo, updateView) {
            var startTime = new Date();
            var result = false;
            if (DCTools20221228.IsDotnetReferenceElement(element) === true) {
                result = rootElement.__DCWriterReference.invokeMethod("SetElementLogicDeleted2", element, logUndo, updateView);
            }
            else {
                result = rootElement.__DCWriterReference.invokeMethod("SetElementLogicDeleted", element, logUndo, updateView);
            }
            DCEventInterfaceLogFunction(rootElement, 'SetElementLogicDeleted', startTime);
            return result;
        };

        /**
         * @name ClearTextHighlight
         * @type function
         * @appnameZh 清空文档文本高亮状态
         * @classification view
         * @change ["2025-3-5","新增接口","wyc" ]
         * @returns ["result","boolean","代表函数是否被成功执行"]
         */
        rootElement.ClearTextHighlight = function () {
            var startTime = new Date();
            var result = rootElement.__DCWriterReference.invokeMethod("ClearTextHighlight");
            DCEventInterfaceLogFunction(rootElement, 'ClearTextHighlight', startTime);
            return result;
        };
        ...
        */
        rootElement.SetAutoZoom = function (callback, eventArgs, noAddAttr) {
            ...
            if (hasAutoZoom != null && (hasAutoZoom == 'true' || hasAutoZoom == 'True')) {
                ...
                // 修复SetAutoZoom自适应可能不生效的问题【DUWRITER5_0-4328】
                if (WriterControl_Task.__Tasks && WriterControl_Task.__Tasks.length > 0) {
                    WriterControl_Task.AddCallbackForCompletedAllTasks(AutoZoomFunc);
                } else {
                    AutoZoomFunc();
                }
            } else {
      ...
      */
       ...
       */
        ...
        */
        rootElement.GetElementById = function (id) {
            ...
            var result = null;
            if (DCTools20221228.IsDotnetReferenceElement(id) === true) {
                result = rootElement.__DCWriterReference.invokeMethod("DCGetElementProperties", id, false);
            } else {
                result = rootElement.__DCWriterReference.invokeMethod("DCGetElementProperties2", id, false);
            }
        ...
        */
        ...
        //wyc20250227:获取表格行内部单元格句柄的快速函数
        rootElement.GetRowCellsHandles = function (id) {
            var startTime = new Date();
            var IsOperateDOM = DCGetAllowOperateDOM(rootElement);
            if (IsOperateDOM === false) {
                //阅读、预览、续打、区域选择，只读时不能使用此接口
            ...
            let resultstr = rootElement.__DCWriterReference.invokeMethod("GetRowCellsHandles", id);
            let result = JSON.parse(resultstr);
            DCEventInterfaceLogFunction(rootElement, 'GetRowCellsHandles', startTime);
            return result;
        };
        ...
        * @param ["id","string","元素编号","","",true]
        * @param ["speicfyparent","string","指定父元素的ID或NativeHandle","","",true]
        * @returns ["result","Array","{DCSoft.Writer.Dom.XTextElementList} 找到的元素对象列表"]
        * @describe 获得文档中所有指定编号的元素对象列表, 查找时ID值区分大小写的。
        */
        rootElement.GetElementsById = function (id, speicfyparent) {
            var startTime = new Date();
            let result = rootElement.__DCWriterReference.invokeMethod("GetElementsById", id, speicfyparent);
       ...
       */
        ...
        */
        ...
        /**
        * @name GetNavigateNodesJson
        * @type function
        * @classification file
        * @apinameZh 获得导航节点信息
        * @change ["2025-04-28","增加接口","wyc" ]
        * @returns ["result","array","导航节点对象集合数组"]
        */
        rootElement.GetNavigateNodesJson = function () {
            var startTime = new Date();
            let result = rootElement.__DCWriterReference.invokeMethod("GetNavigateNodesJson");
            DCEventInterfaceLogFunction(rootElement, 'GetNavigateNodesJson', startTime);
            return result;
        };
       ...
       */
        ...
        */
        rootElement.IsValidateXML = function (xmlContent) {
            ...
            if (xmlContent.startsWith("<XTextDocument") && xmlContent.endsWith("<\/XTextDocument>")) {
      ...
      */
        ...
        */
        ...
        {"Type":"Header",`);
            ...
            if (typeof options === "string") {
                FileFormatStr = options.toLowerCase();
            } else if (typeof options === "object" && FileFormatStr != null) {
                if (typeof options.FileFormat === "string" && options.FileFormat.trim() !== "") {
                    FileFormatStr = options.FileFormat.toLowerCase();
                }
            }
            // 删除ctl.SaveDocumentToString('html')生成的HTML标签之间的空格【DUWRITER5_0-4315】
            // 如果 FileFormatStr 是 "html" 且 content 是字符串，则执行内容清理
            if (FileFormatStr == "html" && typeof content === "string") {
                try {
       ...
       */
            ...
            if (Array.isArray(Value) === true || typeof (Value) === "object" || typeof (Value) === "string") {
                ...
                if (Array.isArray(Value) === true) {
                    for (var i = 0; i < Value.length; i++) {
                        ...
                        if (item instanceof Array) {
                            ...
                            Object.keys(item).forEach(function (key) {
                                //将Value中的换行标签<br/>替换为\n
                                if (item[key] && item[key] && typeof item[key] === 'string') {
                                    item[key] = item[key].replace(/<br\s*\/?>|<\/br>/gi, "\n");
                                }
                            });
                    ...
                    Object.keys(Value).forEach(function (key) {
                        //将Value中的换行标签<br/>替换为\n
                        if (Value[key] && Value[key] && typeof Value[key] === 'string') {
                            Value[key] = Value[key].replace(/<br\s*\/?>|<\/br>/gi, "\n");
                        }
                    });
                    ...
                    Value = Value.replace(/<br\s*\/?>|<\/br>/gi, "\n");
                }
            } else {
                isflag = false;
            }
        ...
        * @param ["isdispose","boolean","标注后台是否执行完后释放对象，默认释放","","",true]
        * @returns ["result","boolean","操作是否成功"]
        */
        rootElement.SetElementAttribute = function (id, attributeName, attributeValue, isdispose = true) {
            var startTime = new Date();

            let result = false;
            if (DCTools20221228.IsDotnetReferenceElement(id) === true) {
                result = rootElement.__DCWriterReference.invokeMethod("SetElementAttribute2", id, attributeName, attributeValue, isdispose);
      ...
      */
       ...
       */
         ...
         * @apinameZh 病程下移
         * @classification subdoc
         * @param ["subID","string","病程id","","也可以为空，为空是将当前病程下移",true]
         */
      ...
      */
        ...
        */
                    ...
                    if (bolShowUI == true) {
                        ...
                        switch (strCommandName.toLocaleLowerCase()) {
                            ...
                            case "insertmedicalexpression":
                                if (!Parameter || Boolean(Parameter.ExpressionStyle) == false) {
                                    //如果不存在医学表达式类型，就弹出可以选择的医学表达式弹框
                                    WriterControl_Dialog.MedicalExpressionDialog(Parameter, rootElement, true);
                                } else {
                                    // 存在就直接弹出对应的写值对话框
                                    WriterControl_Dialog.DCDispatchMedicalExpressionExecuteCommand(Parameter, rootElement, true);
                                }
                                break;
        ...
        /**
         *DCDispatchMedicalExpressionExecuteCommand
         *派发医学表达式命令，打开对应的医学表达式编辑对话框 （内部使用不对外开放）
         * @param options 医学表达式参数对象
         * @param isInsertMode 是否为插入模式
         * @param ele 要插入的元素
         * @change ["2025-03-28","为了区分医学表达式属性对话框和医学表达式编辑对话框","lixinyu" ]
         */
        rootElement.DCDispatchMedicalExpressionExecuteCommand = function (options, isInsertMode = false, ele) {
            if (options && options.ExpressionStyle) {
                WriterControl_Dialog.DCDispatchMedicalExpressionExecuteCommand(options, rootElement, isInsertMode, ele);
            }
        };
        ...
        * @param ["smartmode","boolean","是否精简输出的属性值默认值就不输出了","","",false]
        * @returns ["result","boolean","操作是否成功"]
        * @change ["2023-05-12","和四代接口保持兼容","wyc" ]
        * @change ["2025-3-26","新增精简输出模式","wyc" ]
        */
        rootElement.GetElementProperties = function (element, smartmode = false) {
            var startTime = new Date();
            let result = null;
            if (element === null) {
            ...
            if (typeof (element) === "object" && typeof (element.serializeAsArg) === "function") {
                result = rootElement.__DCWriterReference.invokeMethod("DCGetElementProperties", element, smartmode);
            } else {
                result = rootElement.__DCWriterReference.invokeMethod("DCGetElementProperties2", element, smartmode);
            }
            DCEventInterfaceLogFunction(rootElement, 'GetElementProperties', startTime);
            return result;
        };
       ...
       */
        ...
        */
        ...
        /**
         * @name setDocumentCommentsBackColor
         * @type function
         * @apinameZh 批量设置文档批注的背景色
         * @classification file
         * @param ["color","string/array","表示一系列颜色值的字符串数组，可为单个字符串用英文逗号分隔"]
         * @returns ["result","boolean","执行是否成功"]
         * @change ["2025-2-20","创建API","wyc" ]
         */
        rootElement.setDocumentCommentsBackColor = function (color) {
            var startTime = new Date();
            var result = null;
            result = rootElement.__DCWriterReference.invokeMethod("setDocumentCommentsBackColor", color);
            DCEventInterfaceLogFunction(rootElement, 'setDocumentCommentsBackColor', startTime);
            return result;
        };


        /**
         * @name SetDocumentCommentProperties
         * @type function
         * @apinameZh 设置单个文档批注对象的属性
         * @classification file
         * @param ["refobj","string/number","若为数字则表示引用的批注的索引编号，若为字符串则表壳引用的批注的ID"]
         * @param ["parameter","object","属性对象"]
         * @returns ["result","boolean","执行是否成功"]
         * @change ["2025-2-20","创建API","wyc" ]
         */
        rootElement.SetDocumentCommentProperties = function (refobj, parameter) {
            var startTime = new Date();
            var result = null;
            result = rootElement.__DCWriterReference.invokeMethod("SetDocumentCommentProperties", refobj, parameter);
            DCEventInterfaceLogFunction(rootElement, 'setDocumentCommentsBackColor', startTime);
            return result;
        };
       ...
       */
        ...
        */
        ...
        //     "highlightsearchresult": "false"//查找到目标后上是否高亮显示目标文本默认不高亮
        //     "setsearchresultselection": "true"//查找到目标后上是否选中目标文本默认选中
        //     "highlightbackcolor": "yelllow"//高亮文本的背景色默认为黄
        //     "highlightforecolor": "red"//高亮文本的前景色默认为红
        //     "specifycontainer": "null"//指定查找容器的ID或nativehandle，不指定则全文查找
        // }
        ...
        */
        ...
        //     "specifycontainer": "null"//指定查找容器的ID或nativehandle，不指定则全文查找
        // }
        rootElement.SearchAllText = function (options) {
            var startTime = new Date();
            var result = false;
            result = rootElement.__DCWriterReference.invokeMethod("SearchAllText", options);
            DCEventInterfaceLogFunction(rootElement, 'SearchAllText', startTime);
            return result;
        };
    ...
    */
     ...
     */
         ...
         * @classification file
         * @param ["callBack","function","回调函数，用于接收base64字符串","","",true]
         * @change ["2025-02-13","使用五代前端加密并调用四代转发生成PDF的测试接口","wyc" ]
         */
        rootElement.SaveDocumentToPdfBase64String2 = function (callBack) {
            var startTime = new Date();
            var filecontent = rootElement.__DCWriterReference.invokeMethod("SaveDocumentToString", null);
            var options2 = {
                Files: [filecontent],
                DocumentOptions: rootElement.DocumentOptions
            };
            rootElement.GetPDFByFilesFast(options2, callBack);
        };
        rootElement.GetPDFByFilesFast = function (options, callBack) {
            var str = JSON.stringify(options);
            var data2 = new TextEncoder().encode(str);
        ...
        //wyc20250227:新增单文档快速接口
        rootElement.GetPDFBySingleFiles = function (filexml, callBack) {
            var startTime = new Date();
            //wyc20230625:重写逻辑准备调用第四代接口
            var asycinvoke = false;
            var downloadstr = "false";
            ...
            // 此处对应的服务器代码在 DCWriterForASPNET\Writer\Controls\Web\WC_WASM.cs
            var strUrl = strServicePageUrl + "?wasm=getpdfbysinglefiles&dcbid2022=" + DCTools20221228.GetClientID() +
                "&forceblack=" + forceblack +
                "&downloadstr=" + downloadstr;
            ...
            DCEventInterfaceLogFunction(rootElement, 'GetPDFBySingleFiles', startTime);
            return resultstr;
        };
      ...
      * @param ["isdispose","boolean","标注后台是否执行完后释放对象，默认释放","","",true]
      */
        rootElement.RemoveElementAttributes = function (element, options, isdispose = true) {
            var startTime = new Date();
            var result = false;
            if (DCTools20221228.IsDotnetReferenceElement(element) === true) {
                result = rootElement.__DCWriterReference.invokeMethod("RemoveElementAttributes2", element, options, isdispose);
            } else {
       ...
       */
        ...
        rootElement.SortRootContent = function (sortFunction) {
            ...
            if (changed == true) {
                // 排序时禁用痕迹的代码转移到需要病程移动时才调用【DUWRITER5_0-4211】
                ...
                rootElement.ApplyDocumentOptions();
                // 更新属性后，之前展示的删除痕迹会不展示，需要进行刷新【DUWRITER5_0-4211】
                rootElement.RefreshInnerView(false);
            }
            return changed;
        };
        ...
        */
        ...
        /**
       * @name FocusLineByPageIndexLineIndex
       * @type function
       * @apinameZh 定位或选择指定页的指定行
       * @classification file
       * @param ["pageIndex","number","从0开始计算的页号","true","","false"]
       * @param ["lineIndex","number","从0开始计算的行号。","true","","false"]
       * @param ["isSelect","bool","是否选择整行内容","true","","false"]
       */
        rootElement.FocusLineByPageIndexLineIndex = function (pageIndex, lineIndex, isSelect = false) {
            var startTime = new Date();
            var result = rootElement.__DCWriterReference.invokeMethod("FocusLineByPageIndexLineIndex", pageIndex, lineIndex, isSelect);
            DCEventInterfaceLogFunction(rootElement, 'FocusLineByPageIndexLineIndex', startTime);
            return result;
        };

        /**
        * @name SetDiagnosticTextById
        * @type function
        * @apinameZh 根据ID设置诊断文本
        * @classification structuralelement
        * @param ["idStr","string","元素的ID，用于定位要设置诊断文本的元素，null表示获取当前输入域","","","false"]
        * @param ["DiagnosticTextArray","Array","诊断文本数组，包含要设置的诊断信息;格式为数组，其中包含对象，对象中：mainDX(主诊断),secondDX(数组)(次诊断)","","","false"]
        * @param ["HierarchyMode","string","层级模式，决定次诊断是否合并 Expand：不合并；Merge：合并；默认合并","","","false"]
        * @param ["SymbolStr","string","符号字符串，用于排序，【1,a,A,ⅰ,Ⅰ】默认值为1","1","","false"]
        */
        rootElement.SetDiagnosticTextById = function (idStr, DiagnosticTextArray, HierarchyMode, SymbolStr = "1") {
            // 判断元素
            var el;
            if (typeof idStr === "string") {
                el = rootElement.GetElementByIdExt(idStr);
            } else {
                el = idStr;
            }
            if (!el) {
                el = rootElement.CurrentInputField();
            }
            var elProps = rootElement.GetElementProperties(el);
            if (typeof elProps !== "object" || elProps == null || !elProps.NativeHandle) {
                return false;
            }
            var InsertText = "";
            var elNativeHandle = elProps.NativeHandle;
            if (typeof SymbolStr !== "string") {
                SymbolStr = "1";
            }
            var SymbolCode = SymbolStr.charCodeAt(0);
            if (DiagnosticTextArray && Array.isArray(DiagnosticTextArray)) {
                for (var i = 0; i < DiagnosticTextArray.length; i++) {
                    var listFirstStr = String.fromCharCode(SymbolCode + i);
                    var mainDXStr = DiagnosticTextArray[i].mainDX;
                    var secondDXAarray = DiagnosticTextArray[i].secondDX;
                    if (i != 0) {
                        InsertText += "\n";
                    }
                    InsertText += listFirstStr;
                    InsertText += "、";
                    InsertText += mainDXStr;
                    if (secondDXAarray && Array.isArray(secondDXAarray) && secondDXAarray.length > 0) {
                        InsertText += "\n";
                        InsertText += "   ";
                        for (var j = 0; j < secondDXAarray.length; j++) {
                            var secondDXStr = secondDXAarray[j];
                            InsertText += secondDXStr;
                            if (j != secondDXAarray.length - 1) {
                                // 展开状态下
                                if (HierarchyMode == "Expand") {
                                    InsertText += "\n";
                                    InsertText += "   ";
                                } else {
                                    InsertText += "、";
    ...
    BindDCWriterDocument: function (rootElement) {
        ...
        * @apinameZh 加载文档字符串
        */
        rootElement.LoadDocumentFromBase64String = function (xmlstring) {
        ...
        * @type function
        * @classification file
        * @param { string } format 加载的文档XML字符串。
        * @param { boolean } dispose 保存完是否自动销毁资源。
        * @apinameZh 将文档保存成字符串
        */
        rootElement.SaveDocumentToBase64String = function (format, dispose = false) {
    ...
    BindControlForTemperatureControlForWASM: function (rootElement, refDCWriter) {
        ...
        */
        rootElement.SaveTemperatureDocumentToPDF = function (callback) {
            ...
            //[DUWRITER5_0-4218] 20250307 lxy 解决下载pdf宽高与svg宽高不相同的问题(怀疑是jspdf单位为px时，转mm导致的问题，现在直接在jspdf中使用mm单位)
            ...
            for (var i = 0; i < allSvg.length; i++) {
                ...
                img.onload = function () {
                    ...
                    pdf.addImage(png, 'png', 0, 0, pdfWidth, pdfHeight);// x, y, 宽, 高
